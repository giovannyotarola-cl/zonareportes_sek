...
If(Request("passwordP") <> "") Then
	...
	sqlProfesor = "SELECT * FROM RA_PROFES WHERE RUT = '"&Request("passwordP")&"'"
...
If(Request("passwordA") <> "") Then
	...
	sqlAlumno = "SELECT w.RUT, w.PASSWEB, w.PASSCHANGED, c.CODCLI, c.DIG, c.PATERNO, c.MATERNO, c.NOMBRE FROM MT_WEBMAIL w, MT_CLIENT c WHERE (RUT = '"&Request("passwordA")&"') AND (w.RUT = c.CODCLI) AND (c.CODCLI = '"&Request("passwordA")&"')"
