...
cnIndex.Open "driver={SQL Server};server=192.168.100.226; database=Matricula; uid=matricula;pwd=dtb01s"
	...
	sqlAlumnos = "SELECT MT_CLIENT.PATERNO, MT_CLIENT.MATERNO, MT_CLIENT.NOMBRE, MT_CLIENT.CODCLI, MT_CARRER.NOMBRE_C, MT_CARRER.OTORTITULO, MT_ALUMNO.FECREG, RA_SITU.EMISION FROM  MT_CLIENT INNER JOIN MT_ALUMNO ON MT_CLIENT.CODCLI = MT_ALUMNO.RUT INNER JOIN MT_CARRER ON MT_ALUMNO.CODCARPR = MT_CARRER.CODCARR INNER JOIN RA_FACULTAD ON MT_CARRER.CODFAC = RA_FACULTAD.CODFAC INNER JOIN RA_SITU ON MT_ALUMNO.CODCLI = RA_SITU.CODCLI WHERE (RA_SITU.TIPOSITU = 31) AND (MT_ALUMNO.RUT = '"&request("codcli")&"') "
...
<div id="container" style="margin-top: 140px;">
	<div class="titulo" style="font-size:16px;">
        <h3 style="font-size:17px; letter-spacing:2px; padding-top: 10px;">CERTIFICADO DE TÍTULO</h3></p>
 		<p></p>
    </div>
	<div id="content-center" style="font-size:16px; line-height:20px;">
      ...
      <% 
		   ...
		   Next
	   %>
      <p align="justify">La Universidad  SEK,  inscrita en el Ministerio de Educación, folio C Nº20 del 3 de Octubre de 1988, confirió a Don/Doña <b><%=rsAlumnos("NOMBRE")&" "&rsAlumnos("PATERNO")&" "&rsAlumnos("MATERNO")%></b>, Cédula de Identidad Nº <b><%=Cedula%></b>, con fecha <%= rsAlumnos("EMISION") %>, el Título Profesional de <%= rsAlumnos("OTORTITULO") %>, habiendo obtenido una calificación <strong><%= request("calificacion") %></strong> (Máximo: siete, mínimo: cuatro coma cero), de acuerdo a las disposiciones legales vigentes contenidas en la ley Nº 18.962, Orgánica Constitucional de Enseñanza, Decreto Exento Nº 308 de 1991 del Ministerio de Educación Pública.</p>
<p align="justify">Extendido para acreditar la posesión del respectivo título.    </p>
<% else %>
