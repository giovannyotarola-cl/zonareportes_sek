...
cnIndex.Open "driver={SQL Server};server=192.168.100.226; database=Matricula; uid=matricula;pwd=dtb01s"
	...
	sqlAlumnos = "SELECT MT_CLIENT.PATERNO, MT_CLIENT.MATERNO, MT_CLIENT.NOMBRE,  MT_CLIENT.CODCLI + '-' + MT_CLIENT.DIG AS RUT, MT_ALUMNO.CODCARPR, MT_ALUMNO.CODPESTUD, MT_CARRER.NOMBRE_C, RA_FACULTAD.DESCRIPCION FROM MT_CLIENT INNER JOIN MT_ALUMNO ON MT_CLIENT.CODCLI = MT_ALUMNO.RUT INNER JOIN MT_CARRER ON MT_ALUMNO.CODCARPR = MT_CARRER.CODCARR LEFT OUTER JOIN RA_FACULTAD ON MT_CARRER.CODFAC = RA_FACULTAD.CODFAC WHERE (MT_ALUMNO.RUT = '"&request("codcli")&"' AND MT_ALUMNO.ESTACAD IN('VIGENTE'))"
...
<div id="container" style="margin-top: 60px;">
	<div class="titulo" style="font-size:16px;">
    	<p>
   	  <h3 style="font-size:20px; letter-spacing:8px;">CERTIFICADO</h3></p>
    	</p>
    </div>
	<div id="content-center" style="font-size:16px; line-height:20px;">
	  ...
	  <p align="justify">&nbsp;</p>
    <p align="justify">Que Don/Doña <%= rsAlumnos("NOMBRE")&" "&rsAlumnos("PATERNO")&" "&rsAlumnos("MATERNO") %>, cédula de identidad <%= rsAlumnos("RUT") %>, está matriculado(a) en calidad de alumno(a) regular del <%= semestreActualTexto %> Semestre, Período Académico <%= request("anD") %> en <%= cursoActualTexto %> año de la carrera de <%= rsAlumnos("NOMBRE_C") %> impartida por la <%= rsAlumnos("DESCRIPCION") %>.</p>
    <p align="justify">&nbsp;</p>
		<p align="justify">Se extiende el presente documento a petición del(la) interesado(a) para Asignación Familiar.
  <% else %>
	  </p>
    ...
    <div id="footer" style="font-size:16px;">
    	<div class="certif" style="font-size:16px; text-align: center;">
    	  <p>&nbsp;</p>
    	  <p>&nbsp;</p>
    	  <p>Nolberto Alarcón Carvajal</p>
    	  <p>&nbsp;</p>
