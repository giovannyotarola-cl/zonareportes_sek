...
Public Class ImageConversions
    ...
    Protected Sub CreatePhoto()
        ...
        Try
            ...
            Dim fs As FileStream = New FileStream("F:\\Temp\\" + file_foto + ".bmp", FileMode.OpenOrCreate, FileAccess.Write)
            ...
            Dim image1 As System.Drawing.Image = System.Drawing.Image.FromFile("F:\\Temp\\" + file_foto + ".bmp")
    ...
    Private Sub store_pic_Sql(ByVal img As System.Drawing.Image)
        ...
        conn.ConnectionString = "Data Source=192.168.200.225;Initial Catalog=NAV_UISEK_ECUADOR;Persist Security Info=True;User ID=sa;Password=Sa070507"

        'Comprobar si el alumno está en la tabla de fotos o no para hacer insert o update
        Dim sqlCheck As String = "select No_ from dbo.[Fotos Alumnos] where No_ = '" + alumno + "'"
        da = New SqlDataAdapter(sqlCheck, conn)
        da.Fill(ds)

        If ds.Tables(0).Rows.Count > 0 Then
            sqlInsertar = "update dbo.[Fotos Alumnos] set Picture = @imgData where No_ = '" + alumno + "'"
        Else
            sqlInsertar = "insert into dbo.[Fotos Alumnos] (No_, Picture) values ('" + alumno + "', @imgData)"
        End If
