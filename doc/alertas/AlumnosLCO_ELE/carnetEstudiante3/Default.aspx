    ...
    <asp:SqlDataSource ID="origen_listado_alumnos" runat="server" 
        ...
        SelectCommand="SELECT distinct  MT_CLIENT.NOMBRE, 
                      ...
                      FROM         dbo.foto_alumno RIGHT OUTER JOIN
                       ...
                       --ORDER BY dbo.MT_CLIENT.PATERNO, dbo.MT_CLIENT.MATERNO
UNION ALL
SELECT A.NOMBRES  AS NOMBRE,
       A.AP_PATER AS PATERNO,
       A.AP_MATER AS MATERNO,
       A.RUT,
       (SELECT MAX(VALOR) 
            FROM MT_PARAME_DET WHERE IDPARAMETRO='ANO') AS ANO_MAT,
        '1' AS  P_MAT,
       (SELECT TOP 1 CODCARR AS CODCARR FROM RA_HORPROF
        WHERE CODPROF = A.CODPROF
        AND ANO IN (SELECT MAX(VALOR) 
                    FROM MT_PARAME_DET WHERE IDPARAMETRO='ANO')) AS C_MAT     ,
        '' AS CODCLI,
        CASE WHEN B.FOTO IS NULL THEN 'Sin Foto' ELSE 'Ok' END AS Foto            
FROM dbo.foto_alumno B RIGHT OUTER JOIN
     dbo.RA_PROFES A ON B.CODCLI = A.CODPROF
WHERE LTRIM(RTRIM(A.RUT))+LTRIM(RTRIM(A.DV)) IN (SELECT US_CONSUSER 
										   	     FROM ca_usuarios 
                                                 WHERE CODPROF IN ('6288666','5329812')
                                                 AND US_ESVIGENTE =1)
"></asp:SqlDataSource>
</asp:Content>
