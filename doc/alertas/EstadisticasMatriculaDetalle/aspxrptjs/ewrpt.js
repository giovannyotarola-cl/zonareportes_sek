// JavaScript for ASP.NET Report Maker 4
...
// Shorten the URL
function ewrpt_SubmitForm(table_id) {
	var tbl = document.getElementById(table_id);
	if (!tbl)
		return;
	var qs = {}, q = "", val = "";
	var arEl = tbl.getElementsByTagName("INPUT");
	for (var i=0; i<arEl.length; i++) {
		el = arEl[i];
		val = ewrpt_GetValue(arEl[i]);
		if (val != "")
			qs[arEl[i].name] = encodeURIComponent(val);
	}
	var arEl = tbl.getElementsByTagName("SELECT");
	for (var i=0; i<arEl.length; i++) {
		val = ewrpt_GetValue(arEl[i]);
		if (val != "") 	
			qs[arEl[i].name] = encodeURIComponent(val);
	...
	for (name in qs) {
		if ((name.substr(0,3) == "so1_" && qs["sv1_" + name.substr(4)]) ||
			(name.substr(0,3) == "so2_" && qs["sv2_" + name.substr(4)])
			(name.substr(0,3) == "sc_" && qs["sv1_" + name.substr(4)] && qs["sv2_" + name.substr(4)])) {
			if (q != "") q += "&";
			q += qs[name];	
		} else {
			if (q != "") q += "&";
...
function ewrpt_GetValue(obj) {
	if (!obj)
		return "";
	if (!ewrpt_HasValue(obj))
		return "";
