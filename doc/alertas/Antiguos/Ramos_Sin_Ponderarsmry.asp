...
' ASP Report Maker 3.0+ - Table level configuration (Ramos Sin Ponderar)
' Table Level Constants

Const EW_REPORT_TABLE_VAR = "Ramos_Sin_Ponderar"
Const EW_REPORT_TABLE_SESSION_GROUP_PER_PAGE = "Ramos_Sin_Ponderar_grpperpage"
Const EW_REPORT_TABLE_SESSION_START_GROUP = "Ramos_Sin_Ponderar_start"
Const EW_REPORT_TABLE_SESSION_SEARCH = "Ramos_Sin_Ponderar_search"
Const EW_REPORT_TABLE_SESSION_CHILD_USER_ID = "Ramos_Sin_Ponderar_childuserid"
Const EW_REPORT_TABLE_SESSION_ORDER_BY = "Ramos_Sin_Ponderar_orderby"
...
Ramos Sin Ponderar
<% If sExport = "" Then %>
&nbsp;&nbsp;<a href="Ramos_Sin_Ponderarsmry.asp?export=excel">Export to Excel</a>
<% If bFilterApplied Then %>
&nbsp;&nbsp;<a href="Ramos_Sin_Ponderarsmry.asp?cmd=reset">Reset All Filters</a>
<% End If %>
<% End If %>
<br><br>
<% If sExport = "" Then %>
...
<!-- Search form (begin) -->
<form name="fRamos_Sin_Ponderarsummaryfilter" id="fRamos_Sin_Ponderarsummaryfilter" action="Ramos_Sin_Ponderarsmry.asp" class="ewForm" onSubmit="return ewrpt_ValidateExtFilter(this);">
...
%>
	<tr>
		...
		<select name="sv_FACULTAD" id="sv_FACULTAD"<% If sClearExtFilter = "Ramos_Sin_Ponderar_FACULTAD" Then sClearInputClass = " class=""ewInputCleared""" Else sClearInputClass = "" %>>
...
%>
	...
	<tr>
		...
		<td></td>
		<td colspan="4"><span class="ewRptSearchOpr">
		<select name="sv_CARRERA" id="sv_CARRERA"<% If sClearExtFilter = "Ramos_Sin_Ponderar_CARRERA" Then sClearInputClass = " class=""ewInputCleared""" Else sClearInputClass = "" %>>
...
<% If sClearExtFilter = "Ramos_Sin_Ponderar_CODRAMO" Then sClearInputClass = " class=""ewInputCleared""" Else sClearInputClass = "" %>
	<tr>
		<td><span class="aspreportmaker">CODRAMO</span></td>
		<td><span class="ewrptSearchOpr">contains<input type="hidden" name="so1_CODRAMO" id="so1_CODRAMO" value="LIKE"></span></td>
		<td>
...
<% If sClearExtFilter = "Ramos_Sin_Ponderar_CODSECC" Then sClearInputClass = " class=""ewInputCleared""" Else sClearInputClass = "" %>
	<tr>
...
<% If sClearExtFilter = "Ramos_Sin_Ponderar_ANO" Then sClearInputClass = " class=""ewInputCleared""" Else sClearInputClass = "" %>
	<tr>
...
<% If sClearExtFilter = "Ramos_Sin_Ponderar_PERIODO" Then sClearInputClass = " class=""ewInputCleared""" Else sClearInputClass = "" %>
	<tr>
...
%>
<% If nTotalGrps > 0 Then %>
	<!-- tr><td colspan="15"><span class="aspreportmaker">&nbsp;<br></span></td></tr -->
	<tr class="ewRptGrandSummary"><td colspan="15">Grand Total (<%= FormatNumber(rstotcnt,0) %> Detail Records)</td></tr>
<% End If %>
	</tfoot>
</table>
</div>
...
<form action="Ramos_Sin_Ponderarsmry.asp" name="ewpagerform" id="ewpagerform" class="ewForm">
...
<!--first page button-->
	...
	<td><a href="Ramos_Sin_Ponderarsmry.asp?start=<%= Pager.FirstButton.Start %>"><img src="rptimages/first.gif" alt="First" width="16" height="16" border="0"></a></td>
	<% Else %>
	<td><img src="rptimages/firstdisab.gif" alt="First" width="16" height="16" border="0"></td>
	<% End If %>
<!--previous page button-->
	<% If Pager.PrevButton.Enabled Then %>
	<td><a href="Ramos_Sin_Ponderarsmry.asp?start=<%= Pager.PrevButton.Start %>"><img src="rptimages/prev.gif" alt="Previous" width="16" height="16" border="0"></a></td>
	<% Else %>
...
<!--next page button-->
	...
	<td><a href="Ramos_Sin_Ponderarsmry.asp?start=<%= Pager.NextButton.Start %>"><img src="rptimages/next.gif" alt="Next" width="16" height="16" border="0"></a></td>	
	<% Else %>
	<td><img src="rptimages/nextdisab.gif" alt="Next" width="16" height="16" border="0"></td>
	<% End If %>
<!--last page button-->
	<% If Pager.LastButton.Enabled Then %>
	<td><a href="Ramos_Sin_Ponderarsmry.asp?start=<%= Pager.LastButton.Start %>"><img src="rptimages/last.gif" alt="Last" width="16" height="16" border="0"></a></td>	
	<% Else %>
...
Function GetExtendedFilter()
	...
	ElseIf Request.QueryString("cmd") = "reset" Then
		...
		Call SetSessionDropDownValue(sv_FACULTAD, "Ramos_Sin_Ponderar_FACULTAD")

		' Field CARRERA
		Call SetSessionDropDownValue(sv_CARRERA, "Ramos_Sin_Ponderar_CARRERA")

		' Field CODRAMO
		Call SetSessionFilterValues(sv1_CODRAMO, so1_CODRAMO, sc_CODRAMO, sv2_CODRAMO, so2_CODRAMO, "Ramos_Sin_Ponderar_CODRAMO")

		' Field CODSECC
		Call SetSessionFilterValues(sv1_CODSECC, so1_CODSECC, sc_CODSECC, sv2_CODSECC, so2_CODSECC, "Ramos_Sin_Ponderar_CODSECC")

		' Field ANO
		Call SetSessionFilterValues(sv1_ANO, so1_ANO, sc_ANO, sv2_ANO, so2_ANO, "Ramos_Sin_Ponderar_ANO")

		' Field PERIODO
		Call SetSessionFilterValues(sv1_PERIODO, so1_PERIODO, sc_PERIODO, sv2_PERIODO, so2_PERIODO, "Ramos_Sin_Ponderar_PERIODO")
	...
	Else
		...
		ElseIf Not IsArray(sv_FACULTAD) Then
			If sv_FACULTAD <> EW_REPORT_INIT_VALUE And IsEmpty(Session("sv_Ramos_Sin_Ponderar_FACULTAD")) Then
				bSetupFilter = True ' Set up filter required
			End If
		End If
		...
		ElseIf Not IsArray(sv_CARRERA) Then
			If sv_CARRERA <> EW_REPORT_INIT_VALUE And IsEmpty(Session("sv_Ramos_Sin_Ponderar_CARRERA")) Then
				bSetupFilter = True ' Set up filter required
			End If
		End If
	...
	If bRestoreSession Then
		...
		' Field FACULTAD
		Call GetSessionDropDownValue(sv_FACULTAD, "Ramos_Sin_Ponderar_FACULTAD")

		' Field CARRERA
		Call GetSessionDropDownValue(sv_CARRERA, "Ramos_Sin_Ponderar_CARRERA")

		' Field CODRAMO
		Call GetSessionFilterValues(sv1_CODRAMO, so1_CODRAMO, sc_CODRAMO, sv2_CODRAMO, so2_CODRAMO, "Ramos_Sin_Ponderar_CODRAMO")

		' Field CODSECC
		Call GetSessionFilterValues(sv1_CODSECC, so1_CODSECC, sc_CODSECC, sv2_CODSECC, so2_CODSECC, "Ramos_Sin_Ponderar_CODSECC")

		' Field ANO
		Call GetSessionFilterValues(sv1_ANO, so1_ANO, sc_ANO, sv2_ANO, so2_ANO, "Ramos_Sin_Ponderar_ANO")

		' Field PERIODO
		Call GetSessionFilterValues(sv1_PERIODO, so1_PERIODO, sc_PERIODO, sv2_PERIODO, so2_PERIODO, "Ramos_Sin_Ponderar_PERIODO")
	End If
...
Function getSort()
	...
	If Request.QueryString("cmd").Count > 0 Then
		...
		If sCmd = "resetsort" Then
			...
			Session("sort_Ramos_Sin_Ponderar_FACULTAD") = ""
			Session("sort_Ramos_Sin_Ponderar_CARRERA") = ""
			Session("sort_Ramos_Sin_Ponderar_RAMO") = ""
			Session("sort_Ramos_Sin_Ponderar_CODRAMO") = ""
			Session("sort_Ramos_Sin_Ponderar_CODSECC") = ""
			Session("sort_Ramos_Sin_Ponderar_CODPROF") = ""
			Session("sort_Ramos_Sin_Ponderar_RUT") = ""
			Session("sort_Ramos_Sin_Ponderar_DV") = ""
			Session("sort_Ramos_Sin_Ponderar_AP_PATER") = ""
			Session("sort_Ramos_Sin_Ponderar_AP_MATER") = ""
			Session("sort_Ramos_Sin_Ponderar_NOMBRES") = ""
			Session("sort_Ramos_Sin_Ponderar_ANO") = ""
			Session("sort_Ramos_Sin_Ponderar_PERIODO") = ""
			Session("sort_Ramos_Sin_Ponderar_PORCERT1") = ""
			Session("sort_Ramos_Sin_Ponderar_np") = ""
		End If
