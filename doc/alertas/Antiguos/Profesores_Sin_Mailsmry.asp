...
' ASP Report Maker 3.0+ - Table level configuration (Profesores Sin Mail)
' Table Level Constants

Const EW_REPORT_TABLE_VAR = "Profesores_Sin_Mail"
Const EW_REPORT_TABLE_SESSION_GROUP_PER_PAGE = "Profesores_Sin_Mail_grpperpage"
Const EW_REPORT_TABLE_SESSION_START_GROUP = "Profesores_Sin_Mail_start"
Const EW_REPORT_TABLE_SESSION_SEARCH = "Profesores_Sin_Mail_search"
Const EW_REPORT_TABLE_SESSION_CHILD_USER_ID = "Profesores_Sin_Mail_childuserid"
Const EW_REPORT_TABLE_SESSION_ORDER_BY = "Profesores_Sin_Mail_orderby"
...
<% If sExport = "" Then %>
&nbsp;&nbsp;<a href="Profesores_Sin_Mailsmry.asp?export=excel"><img src="../../images/Talwayseb - Excel 2007.png" border="0" title="Exportar a Excel" /></a>
<% End If %>
<br><br>
<% If sExport = "" Then %>
</div></td></tr>
...
&nbsp;&nbsp;<a href="Profesores_Sin_Mailsmry.asp?cmd=reset">Resetear Filtros</a>
<% End If %>
<br><br>
<div id="ewrptExtFilterPanel"<%= sDivDisplay %>>
<!-- Search form (begin) -->
<form name="fProfesores_Sin_Mailsummaryfilter" id="fProfesores_Sin_Mailsummaryfilter" action="Profesores_Sin_Mailsmry.asp" class="ewForm" onSubmit="return ewrpt_ValidateExtFilter(this);">
...
%>
	<tr>
		...
		<select name="sv_FACULTAD" id="sv_FACULTAD"<% If sClearExtFilter = "Profesores_Sin_Mail_FACULTAD" Then sClearInputClass = " class=""ewInputCleared""" Else sClearInputClass = "" %>>
...
%>
	...
	<tr>
		...
		<td><span class="ewrptSearchOpr">:</span></td>
		<td colspan="4"><span class="ewRptSearchOpr">
		<select name="sv_CARRERA" id="sv_CARRERA"<% If sClearExtFilter = "Profesores_Sin_Mail_CARRERA" Then sClearInputClass = " class=""ewInputCleared""" Else sClearInputClass = "" %>>
...
<% If nTotalGrps > 0 Then %>
	...
	<tr class="ewRptGrandSummary">
	  <td colspan="10">Total Generall (<%= FormatNumber(rstotcnt,0) %> Registros)</td></tr>
<% End If %>
	</tfoot>
</table>
</div>
...
<form action="Profesores_Sin_Mailsmry.asp" name="ewpagerform" id="ewpagerform" class="ewForm">
...
<!--first page button-->
	...
	<td><a href="Profesores_Sin_Mailsmry.asp?start=<%= Pager.FirstButton.Start %>"><img src="rptimages/first.gif" alt="First" width="16" height="16" border="0"></a></td>
	<% Else %>
	<td><img src="rptimages/firstdisab.gif" alt="First" width="16" height="16" border="0"></td>
	<% End If %>
<!--previous page button-->
	<% If Pager.PrevButton.Enabled Then %>
	<td><a href="Profesores_Sin_Mailsmry.asp?start=<%= Pager.PrevButton.Start %>"><img src="rptimages/prev.gif" alt="Previous" width="16" height="16" border="0"></a></td>
	<% Else %>
...
<!--next page button-->
	...
	<td><a href="Profesores_Sin_Mailsmry.asp?start=<%= Pager.NextButton.Start %>"><img src="rptimages/next.gif" alt="Next" width="16" height="16" border="0"></a></td>	
	<% Else %>
	<td><img src="rptimages/nextdisab.gif" alt="Next" width="16" height="16" border="0"></td>
	<% End If %>
<!--last page button-->
	<% If Pager.LastButton.Enabled Then %>
	<td><a href="Profesores_Sin_Mailsmry.asp?start=<%= Pager.LastButton.Start %>"><img src="rptimages/last.gif" alt="Last" width="16" height="16" border="0"></a></td>	
	<% Else %>
...
Function GetExtendedFilter()
	...
	ElseIf Request.QueryString("cmd") = "reset" Then
		...
		Call SetSessionDropDownValue(sv_FACULTAD, "Profesores_Sin_Mail_FACULTAD")

		' Field CARRERA
		Call SetSessionDropDownValue(sv_CARRERA, "Profesores_Sin_Mail_CARRERA")
	...
	Else
		...
		ElseIf Not IsArray(sv_FACULTAD) Then
			If sv_FACULTAD <> EW_REPORT_INIT_VALUE And IsEmpty(Session("sv_Profesores_Sin_Mail_FACULTAD")) Then
				bSetupFilter = True ' Set up filter required
			End If
		End If
		...
		ElseIf Not IsArray(sv_CARRERA) Then
			If sv_CARRERA <> EW_REPORT_INIT_VALUE And IsEmpty(Session("sv_Profesores_Sin_Mail_CARRERA")) Then
				bSetupFilter = True ' Set up filter required
			End If
		End If
	End If
	...
	If bRestoreSession Then
		...
		' Field FACULTAD
		Call GetSessionDropDownValue(sv_FACULTAD, "Profesores_Sin_Mail_FACULTAD")

		' Field CARRERA
		Call GetSessionDropDownValue(sv_CARRERA, "Profesores_Sin_Mail_CARRERA")
	End If
...
Function getSort()
	...
	If Request.QueryString("cmd").Count > 0 Then
		...
		If sCmd = "resetsort" Then
			...
			Session("sort_Profesores_Sin_Mail_FACULTAD") = ""
			Session("sort_Profesores_Sin_Mail_CARRERA") = ""
			Session("sort_Profesores_Sin_Mail_RUT") = ""
			Session("sort_Profesores_Sin_Mail_PATERNO") = ""
			Session("sort_Profesores_Sin_Mail_MATERNO") = ""
			Session("sort_Profesores_Sin_Mail_NOMBRES") = ""
			Session("sort_Profesores_Sin_Mail_ANO") = ""
			Session("sort_Profesores_Sin_Mail_PERIODO") = ""
			Session("sort_Profesores_Sin_Mail_Mail") = ""
			Session("sort_Profesores_Sin_Mail_FONO") = ""
		End If
