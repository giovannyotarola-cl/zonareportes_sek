...
namespace AutoPostulacion
{
    public class Conexion
    {
        ...
        private static SqlDataAdapter BuscarDatos(string TipoDocumento,string Rut, string Tipo)
        {
            ...
            else if (TipoDocumento == "BUSCAINDICA")
            {
                sql = "EXEC SEK_POSTULACION_ONLINE_BUSCADATOS '2018' ";
            }
            else if (TipoDocumento == "ANOPERIODO")
            {
                sql = "EXEC SEK_POSTULACION_ANOPERIODO";
            }
            ...
            else if (TipoDocumento == "COMBOBOX")
            {
                sql = "exec SEK_POSTULACION_ONLINE_LLENA_COMBOBOX '" + Rut + "'";
            }
            else if (TipoDocumento == "EJECUTAR_SQL")
            {
            ...
            else if (TipoDocumento == "REVISA_DEUDA")
            {
                ...
                //sql = "select 'DEUDA-NO/MATRICULADO-NO' as ESTADO";
            }
            else if (TipoDocumento == "BUSCADOCUMENTO")
            {
                //BUSCA LOS DOCUMENTOS
                sql = "exec SEK_REGISTRO_BUSCADOCUMENTO '" + Rut + "'";             
            }
            ...
            {
                sql = "exec SP_DOC_DETALLE_PAGO_Folios '" + Rut + "','" + Tipo + "'";
            }
            ...
            else if (TipoDocumento == "POSTULACION_ONLINE_USUARIOS")
            {
                sql = "select * from sek_registro_postulacion_online ";
                sql = sql + "where RUTAlumno = '" + Rut + "' and rutaArchivo <> ''";
            }
        ...
        public static string GuardarEstados(string rut, string ano, string periodo, string tipo, string observacion, string ruta, string estHistorico, string estadoActual,string estadoPostul)
        {
            if(estadoPostul== "")
            {
                estadoPostul = null;
            ...
            sql = "exec SEK_REGISTRO_ONLINE_INGRESAESTADO '" + rut + "','" + ano + "','" + periodo + "','" + tipo + "','" + observacion + "','" + ruta + "','" + estHistorico + "','" + estadoActual + "','" + estadoPostul + "' ";
        ...
        public static string crearFolio(string folio, string rut, string fecemision, string fecvencimiento,double monto,string banco, int AnoActual, int PeriodoActual)
        {
        ...
        public static string actualizarFechas(string ano_actual, string semestre_actual, string fecha_max_ultima_cuota)
        {
            string tabla = "SEK_POSTULACION_ONLINE_FECHAS";
            string sql = null;
            string messages = string.Empty;
            SqlCommand comando = new SqlCommand();
            SqlConnection myConnection = CreateConnection();
            ...
            sql = "Update " + tabla + " ";
            sql = sql + " set ANO_ACTUAL = " + "'" + ano_actual + "',";
            sql = sql + " SEMESTRE_ACTUAL = " + "'" + semestre_actual + "',";
            sql = sql + " FECHA_MAX_ULTIMA_CUOTA = " + "'" + fecha_max_ultima_cuota + "'";
            // crecion de excepcion de la consulta a la base de datos
            try
            {
                comando.Connection = myConnection;
