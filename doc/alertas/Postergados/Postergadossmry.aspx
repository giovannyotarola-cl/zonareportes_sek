<%@ Page ClassName="Postergadossmry" Language="VB" MasterPageFile="rmasterpage.master" Inherits="AspNetReportMaker4_postergados" %>
<%@ Import Namespace="System" %>
<%@ Import Namespace="System.Collections" %>
<%@ Import Namespace="System.Collections.Generic" %>
<%@ Import Namespace="System.Collections.Specialized" %>
<%@ Import Namespace="System.Web" %>
<%@ Import Namespace="System.Web.UI" %>
<%@ Import Namespace="System.Data" %>
<%@ Import Namespace="System.Data.Common" %>
<%@ Import Namespace="System.Xml" %>
<%@ Import Namespace="System.IO" %>
<%@ Import Namespace="System.Security.Cryptography" %>
<%@ Import Namespace="System.Text" %>
<%@ Import Namespace="System.Text.RegularExpressions" %>
<%@ Import Namespace="System.Drawing" %>
<%@ Import Namespace="System.Drawing.Imaging" %>
<%@ Import Namespace="System.Drawing.Drawing2D" %>
<%@ Import Namespace="System.Reflection" %>
<%@ Import Namespace="System.Net" %>
<%@ Import Namespace="System.Net.Mail" %>
<%@ Import Namespace="System.Net.Mime" %>
<%@ Import Namespace="System.Globalization" %>
<%@ Import Namespace="Microsoft.VisualBasic" %>
<%@ Import Namespace="System.Data.SqlClient" %>
<script runat="server">

	' Table object
	Public Postergados As crPostergados = Nothing

	'
	' Table class (for Postergados)
	'
	Public Class crPostergados
		Inherits AspNetReportMakerBase

		Public ShowCurrentFilter As Boolean = EWRPT_SHOW_CURRENT_FILTER

		Public FilterPanelOption As Integer = EWRPT_FILTER_PANEL_OPTION

		Public CurrentOrder As String = "" ' Current order

		Public CurrentOrderType As String = ""	' Current order type

		' Define table level constants
		Public UseTokenInUrl As Boolean = EWRPT_USE_TOKEN_IN_URL

		' Table variable
		Public ReadOnly Property TableVar() As String
			Get
				Return "Postergados"
			End Get
		End Property

		' Table name
		Public ReadOnly Property TableName() As String
			Get
				Return "Postergados"
			End Get
		End Property

		' Table type
		Public ReadOnly Property TableType() As String
			Get
				Return "REPORT"
			End Get
		End Property

		' Table caption
		Public Function TableCaption() As String
			Return ReportLanguage.TablePhrase(TableVar, "TblCaption")
		End Function

		' Session Group Per Page
		Public Property GroupPerPage() As Integer
			Get
				Return ew_ConvertToInt(ew_Session(EWRPT_PROJECT_NAME + "_" + TableVar + "_grpperpage"))
			End Get
			Set
				ew_Session(EWRPT_PROJECT_NAME + "_" + TableVar + "_grpperpage") = value
			End Set
		End Property

		' Session Start Group
		Public Property StartGroup() As Integer
			Get
				Return ew_ConvertToInt(ew_Session(EWRPT_PROJECT_NAME + "_" + TableVar + "_start"))
			End Get
			Set
				ew_Session(EWRPT_PROJECT_NAME + "_" + TableVar + "_start") = value
			End Set
		End Property

		' Session Order By
		Public Property OrderBy() As String
			Get
				Return Convert.ToString(ew_Session(EWRPT_PROJECT_NAME + "_" + TableVar + "_orderby"))
			End Get
			Set
				ew_Session(EWRPT_PROJECT_NAME + "_" + TableVar + "_orderby") = value
			End Set
		End Property	

		Public Fields As New Dictionary(Of String, crField)()

		Public Rut As crField

		Public Nombre As crField

		Public Paterno As crField

		Public Materno As crField

		Public Codigo_Carrera As crField

		Public AF1o As crField

		Public Periodo As crField

		Public Codigo As crField

		Public Estado_Academico As crField

		Public Estado_Final As crField

		Public Descripcion As crField

		Public Fecha_ResoluciF3n As crField

		Public ObservaciF3n As crField

		Public Export As String = ""	' Export

		Public ExportAll As Boolean = False

		Public RowType As Integer	' Row type

		Public RowTotalType As Integer	' Row total type

		Public RowTotalSubType As Integer	' Row total subtype

		Public RowGroupLevel As Integer	' Row group level

		Public RowAttrs As New Hashtable()	' Row attributes

		' Reset CSS styles for table object
		Public Sub ResetCSS()
			RowAttrs("style") = ""
			RowAttrs("class") = ""
			For Each fld As KeyValuePair(Of String, crField) In Fields
				fld.Value.ResetCSS()
			Next
		End Sub		

		'
		' Table class constructor
		'		
		Public Sub New(APage As AspNetReportMakerPage)
			m_Page = APage
			m_ParentPage = APage.ParentPage

			' Rut
			Rut = new crField("Postergados", "Postergados", "x_Rut", "Rut", "[Rut]", 200, EWRPT_DATATYPE_STRING, -1)
			Rut.Page = APage
			Rut.ParentPage = APage.ParentPage
			Fields.Add("Rut", Rut)
			Rut.DateFilter = ""
			Rut.SqlSelect = ""
			Rut.SqlOrderBy = ""

			' Nombre
			Nombre = new crField("Postergados", "Postergados", "x_Nombre", "Nombre", "[Nombre]", 200, EWRPT_DATATYPE_STRING, -1)
			Nombre.Page = APage
			Nombre.ParentPage = APage.ParentPage
			Fields.Add("Nombre", Nombre)
			Nombre.DateFilter = ""
			Nombre.SqlSelect = ""
			Nombre.SqlOrderBy = ""

			' Paterno
			Paterno = new crField("Postergados", "Postergados", "x_Paterno", "Paterno", "[Paterno]", 200, EWRPT_DATATYPE_STRING, -1)
			Paterno.Page = APage
			Paterno.ParentPage = APage.ParentPage
			Fields.Add("Paterno", Paterno)
			Paterno.DateFilter = ""
			Paterno.SqlSelect = ""
			Paterno.SqlOrderBy = ""

			' Materno
			Materno = new crField("Postergados", "Postergados", "x_Materno", "Materno", "[Materno]", 200, EWRPT_DATATYPE_STRING, -1)
			Materno.Page = APage
			Materno.ParentPage = APage.ParentPage
			Fields.Add("Materno", Materno)
			Materno.DateFilter = ""
			Materno.SqlSelect = ""
			Materno.SqlOrderBy = ""

			' Codigo Carrera
			Codigo_Carrera = new crField("Postergados", "Postergados", "x_Codigo_Carrera", "Codigo Carrera", "[Codigo Carrera]", 200, EWRPT_DATATYPE_STRING, -1)
			Codigo_Carrera.Page = APage
			Codigo_Carrera.ParentPage = APage.ParentPage
			Fields.Add("Codigo_Carrera", Codigo_Carrera)
			Codigo_Carrera.DateFilter = ""
			Codigo_Carrera.SqlSelect = ""
			Codigo_Carrera.SqlOrderBy = ""

			' Año
			AF1o = new crField("Postergados", "Postergados", "x_AF1o", "Año", "[Año]", 131, EWRPT_DATATYPE_NUMBER, -1)
			AF1o.Page = APage
			AF1o.ParentPage = APage.ParentPage
			AF1o.FldDefaultErrMsg = ReportLanguage.Phrase("IncorrectFloat")
			Fields.Add("AF1o", AF1o)
			AF1o.DateFilter = ""
			AF1o.SqlSelect = ""
			AF1o.SqlOrderBy = ""

			' Periodo
			Periodo = new crField("Postergados", "Postergados", "x_Periodo", "Periodo", "[Periodo]", 131, EWRPT_DATATYPE_NUMBER, -1)
			Periodo.Page = APage
			Periodo.ParentPage = APage.ParentPage
			Periodo.FldDefaultErrMsg = ReportLanguage.Phrase("IncorrectFloat")
			Fields.Add("Periodo", Periodo)
			Periodo.DateFilter = ""
			Periodo.SqlSelect = ""
			Periodo.SqlOrderBy = ""

			' Codigo
			Codigo = new crField("Postergados", "Postergados", "x_Codigo", "Codigo", "[Codigo]", 131, EWRPT_DATATYPE_NUMBER, -1)
			Codigo.Page = APage
			Codigo.ParentPage = APage.ParentPage
			Codigo.FldDefaultErrMsg = ReportLanguage.Phrase("IncorrectFloat")
			Fields.Add("Codigo", Codigo)
			Codigo.DateFilter = ""
			Codigo.SqlSelect = ""
			Codigo.SqlOrderBy = ""

			' Estado Academico
			Estado_Academico = new crField("Postergados", "Postergados", "x_Estado_Academico", "Estado Academico", "[Estado Academico]", 200, EWRPT_DATATYPE_STRING, -1)
			Estado_Academico.Page = APage
			Estado_Academico.ParentPage = APage.ParentPage
			Fields.Add("Estado_Academico", Estado_Academico)
			Estado_Academico.DateFilter = ""
			Estado_Academico.SqlSelect = ""
			Estado_Academico.SqlOrderBy = ""

			' Estado Final
			Estado_Final = new crField("Postergados", "Postergados", "x_Estado_Final", "Estado Final", "[Estado Final]", 200, EWRPT_DATATYPE_STRING, -1)
			Estado_Final.Page = APage
			Estado_Final.ParentPage = APage.ParentPage
			Fields.Add("Estado_Final", Estado_Final)
			Estado_Final.DateFilter = ""
			Estado_Final.SqlSelect = ""
			Estado_Final.SqlOrderBy = ""

			' Descripcion
			Descripcion = new crField("Postergados", "Postergados", "x_Descripcion", "Descripcion", "[Descripcion]", 200, EWRPT_DATATYPE_STRING, -1)
			Descripcion.Page = APage
			Descripcion.ParentPage = APage.ParentPage
			Fields.Add("Descripcion", Descripcion)
			Descripcion.DateFilter = ""
			Descripcion.SqlSelect = ""
			Descripcion.SqlOrderBy = ""

			' Fecha Resolución
			Fecha_ResoluciF3n = new crField("Postergados", "Postergados", "x_Fecha_ResoluciF3n", "Fecha Resolución", "[Fecha Resolución]", 135, EWRPT_DATATYPE_DATE, 7)
			Fecha_ResoluciF3n.Page = APage
			Fecha_ResoluciF3n.ParentPage = APage.ParentPage
			Fecha_ResoluciF3n.FldDefaultErrMsg = ReportLanguage.Phrase("IncorrectDateDMY").Replace("%s", "/")
			Fields.Add("Fecha_ResoluciF3n", Fecha_ResoluciF3n)
			Fecha_ResoluciF3n.DateFilter = ""
			Fecha_ResoluciF3n.SqlSelect = ""
			Fecha_ResoluciF3n.SqlOrderBy = ""

			' Observación
			ObservaciF3n = new crField("Postergados", "Postergados", "x_ObservaciF3n", "Observación", "[Observación]", 200, EWRPT_DATATYPE_STRING, -1)
			ObservaciF3n.Page = APage
			ObservaciF3n.ParentPage = APage.ParentPage
			Fields.Add("ObservaciF3n", ObservaciF3n)
			ObservaciF3n.DateFilter = ""
			ObservaciF3n.SqlSelect = ""
			ObservaciF3n.SqlOrderBy = ""
		End Sub

		' Single column sort
		Public Sub UpdateSort(ofld As crField)
			If (CurrentOrder = ofld.FldName) Then
				Dim sLastSort As String = ofld.Sort
				Dim sThisSort As String
				If (CurrentOrderType = "ASC" OrElse CurrentOrderType = "DESC") Then
					sThisSort = CurrentOrderType
				Else
					sThisSort = IIf(sLastSort = "ASC", "DESC", "ASC")
				End If
				ofld.Sort = sThisSort
			Else
				If (ofld.GroupingFieldId = 0) Then ofld.Sort = ""
			End If
		End Sub

		' Get Sort SQL
		Public Function SortSql() As String
			Dim sDtlSortSql As String = ""
			Dim argrps As New List(Of String)()
			For Each field As KeyValuePair(Of String, crField) In Fields
				Dim fld As crField = field.Value
				If ew_NotEmpty(fld.Sort) Then
					If fld.GroupingFieldId > 0 Then
						If ew_NotEmpty(fld.FldGroupSql) Then
							argrps.Add(fld.FldGroupSql.Replace("%s", fld.FldExpression) + " " + fld.Sort)
						Else
							argrps.Add(fld.FldExpression + " " + fld.Sort)
						End If
					Else
						If ew_NotEmpty(sDtlSortSql) Then
							sDtlSortSql += ", "
						End If
						sDtlSortSql += fld.FldExpression + " " + fld.Sort
					End If
				End If
			Next
			Dim sSortSql As String = ""
			For Each grp As String In argrps
				If ew_NotEmpty(sSortSql) Then
					sSortSql += ", "
				End If
				sSortSql += grp
			Next
			If ew_NotEmpty(sDtlSortSql) Then
				If ew_NotEmpty(sSortSql) Then
					sSortSql += ", "
				End If
				sSortSql += sDtlSortSql
			End If
			Return sSortSql
		End Function

		' Table level SQL
		Public ReadOnly Property SqlFrom As String ' From
			Get
				Return "[SEK_Postergados]"
			End Get
		End Property

		Public ReadOnly Property SqlSelect As String ' Select
			Get
				Return "SELECT * FROM " + SqlFrom
			End Get
		End Property

		Public ReadOnly Property SqlWhere As String ' Where
			Get
				Return ""
			End Get
		End Property

		Public ReadOnly Property SqlGroupBy As String ' Group By
			Get
				Return ""
			End Get
		End Property

		Public ReadOnly Property SqlHaving As String ' Having
			Get
				Return ""
			End Get
		End Property

		Public ReadOnly Property SqlOrderBy As String ' Order By
			Get
				Return ""
			End Get
		End Property

		' Table Level Group SQL
		Public ReadOnly Property SqlFirstGroupField As String
			Get
				Return ""
			End Get
		End Property

		Public ReadOnly Property SqlSelectGroup As String
			Get
				Return "SELECT DISTINCT " + SqlFirstGroupField + " FROM " + SqlFrom
			End Get
		End Property

		Public ReadOnly Property SqlOrderByGroup As String
			Get
				Return ""
			End Get
		End Property

		Public ReadOnly Property SqlSelectAgg As String
			Get
				Return "SELECT * FROM " + SqlFrom
			End Get
		End Property

		Public ReadOnly Property SqlAggPfx As String
			Get
				Return ""
			End Get
		End Property

		Public ReadOnly Property SqlAggSfx As String
			Get
				Return ""
			End Get
		End Property

		Public ReadOnly Property SqlSelectCount As String
			Get
				Return "SELECT COUNT(*) FROM " + SqlFrom
			End Get
		End Property

		' Sort URL
		Public Function SortUrl(fld As crField) As String
			Return ""
		End Function

		' Row attributes
		Public Function RowAttributes() As String
			Dim sAtt As String = ""
			For Each Attr As DictionaryEntry In RowAttrs
				If ew_NotEmpty(Attr.Value) Then
					sAtt += " " + Attr.Key + "=""" + Convert.ToString(Attr.Value).Trim() + """"
				End If
			Next
			Return sAtt
		End Function

		' Table level events
		' Row Rendering event
		Public Sub Row_Rendering()

			' Enter your code here	
		End Sub

		' Row Rendered event
		Public Sub Row_Rendered()

			' To view properties of field class, use:
			' ew_Write(<FieldName>.AsString())

		End Sub

		' Load Custom Filters event
		Public Sub CustomFilters_Load()

			'Enter your code here	
			'ewrpt_RegisterCustomFilter(<Field>, "LastMonth", "Last Month", "GetLastMonthFilter") ' Date example
			'ewrpt_RegisterCustomFilter(<Field>, "StartsWithA", "Starts With A", "GetStartsWithAFilter") ' String example

		End Sub

		' Page Filter Validated event
		Public Sub Page_FilterValidated()

			'Example:
			'MyField1.SearchValue = "your search criteria" ' Search value

		End Sub

		' Chart Rendering event
		Public Sub Chart_Rendering(ByRef chart As Object)

			'Dim cht As crChart = CType(chart, crChart) ' Convert to crChart type or
			'Dim cht As crGantt = CType(chart, crGantt) ' Convert to crGantt type
			' Enter your code here

		End Sub

		' Chart Rendered event
		Public Sub Chart_Rendered(chart As Object, ByRef chartxml As String)

			'Dim cht As crChart = CType(chart, crChart) ' Convert to crChart type or
			'Dim cht As crGantt = CType(chart, crGantt) ' Convert to crGantt type
			' Enter your code here

		End Sub

		' Email Sending event
		Public Function Email_Sending(ByRef Email As crEmail, Args As Hashtable) As Boolean

			'ew_Write(Email.AsString())
			'HttpContext.Current.Response.End()

			Return True
		End Function
	End Class

	' Page object
	Public Postergados_summary As crPostergados_summary

	'
	' Page class
	'
	Public Class crPostergados_summary
		Inherits AspNetReportMakerPage
		Implements IDisposable

		' Page URL
		Public Function PageUrl() As String
			Dim sUrl As String = ew_CurrentPage() & "?"
			If (Postergados.UseTokenInUrl) Then
				sUrl &= "t=" & Postergados.TableVar & "&" ' Add page token
			End If
			Return sUrl
		End Function

		' Export URLs
		Public ExportPrintUrl As String = ""

		Public ExportExcelUrl As String = ""

		Public ExportWordUrl As String = ""

		' Message
		Public Property Message As String
			Get
				Return Convert.ToString(ew_Session(EWRPT_SESSION_MESSAGE))
			End Get	
			Set(ByVal Value As String)
				If (ew_NotEmpty(ew_Session(EWRPT_SESSION_MESSAGE))) Then ' Append
					ew_Session(EWRPT_SESSION_MESSAGE) &= "<br>" & value
				Else
					ew_Session(EWRPT_SESSION_MESSAGE) = Value
				End If
			End Set
		End Property

		' Show message
		Public Sub ShowMessage()
			Dim sMessage As String = Message
			Message_Showing(sMessage)
			If (ew_NotEmpty(sMessage)) Then ' Message in Session, display
				ew_Write("<div class=""ewMessage"">" & sMessage & "</div>")
				ew_Session(EWRPT_SESSION_MESSAGE) = "" ' Clear message in Session
			End If
		End Sub

		Public PageHeader As String = ""

		Public PageFooter As String = ""

		' Show Page Header
		Public Sub ShowPageHeader()
			Dim sHeader As String = PageHeader
			Page_DataRendering(sHeader)
			If (ew_NotEmpty(sHeader)) Then ' Header exists, display
				ew_Write("<p><span class=""aspnetreportmaker"">" & sHeader & "</span></p>")
			End If
		End Sub

		' Show Page Footer
		Public Sub ShowPageFooter()
			Dim sFooter As String = PageFooter
			Page_DataRendered(sFooter)
			If (ew_NotEmpty(sFooter)) Then ' Footer exists, display
				ew_Write("<p><span class=""aspnetreportmaker"">" & sFooter & "</span></p>")
			End If
		End Sub

		' Validate page request
		Public Function IsPageRequest() As Boolean
			If (Postergados.UseTokenInUrl) Then
				If (HttpContext.Current.Request.RequestType = "POST") Then
					Return (ew_SameStr(Postergados.TableVar, ew_Post("t")))
				End If
				If (ew_NotEmpty(ew_Get("t"))) Then
					Return (ew_SameStr(Postergados.TableVar, ew_Get("t")))
				End If
			End If
			Return True
		End Function

		' ASP.NET page object
		Public ReadOnly Property AspNetPage As Postergadossmry 
			Get
				Return CType(m_ParentPage, Postergadossmry)
			End Get
		End Property

		' Table object (Postergados)
		Public Property Postergados As crPostergados 
			Get		
				Return AspNetPage.Postergados ' Unlike ASP.NET Maker, the table object is not in the base class.
			End Get
			Set(ByVal Value As crPostergados)
				AspNetPage.Postergados = Value	
			End Set	
		End Property

		'
		' Page class constructor
		'
		Public Sub New(APage As AspNetReportMaker4_postergados)		
			m_ParentPage = APage
			m_Page = Me
			m_PageID = "summary"
			m_PageObjName = "Postergados_summary"
			m_PageObjTypeName = "crPostergados_summary"

			' Table name
			m_TableName = "Postergados"

			' Language object
			ReportLanguage = New crLanguage(Me)

			' Table object (Postergados)
			Postergados = New crPostergados(Me)			

			' Initialize URLs
			ExportPrintUrl = PageUrl() & "export=print"
			ExportExcelUrl = PageUrl() & "export=excel"
			ExportWordUrl = PageUrl() & "export=word"

			' Open connection
			Conn = New cConnection()
		End Sub

		' 
		'  Page_Init
		'
		Public Sub Page_Init()

			' Get export parameters
			If (ew_NotEmpty(ew_Get("export"))) Then
				Postergados.Export = ew_Get("export")
			End If
			gsExport = Postergados.Export ' Get export parameter, used in header
			gsExportFile = Postergados.TableVar ' Get export file, used in header
			If (Postergados.Export = "excel") Then
				HttpContext.Current.Response.ContentType = "application/vnd.ms-excel;charset=utf-8"
				HttpContext.Current.Response.AddHeader("Content-Disposition", "attachment; filename=" & gsExportFile & ".xls")	
			End If

			' Global Page Loading event (in userfn*.aspx)
			ParentPage.Page_Loading()

			' Page Load event
			Page_Load()
		End Sub

		'
		'  Class terminate
		'
		Public Sub Dispose() Implements IDisposable.Dispose
			Page_Terminate("")
		End Sub

		'
		' Page_Terminate
		'
		Public Sub Page_Terminate(url As String)

			' Page Unload event
			Page_Unload()

			' Global Page Unloaded event (in userfn*.aspx)
			ParentPage.Page_Unloaded()

			' Close connection
			Conn.Dispose()

			' Go to URL if specified
			If (ew_NotEmpty(url)) Then
				HttpContext.Current.Response.Clear()
				HttpContext.Current.Response.Redirect(url)
			End If			
		End Sub

		' Temp variables
		Public dr As SqlDataReader	' DataReader

		Public rs As New ArrayList()

		Public rsgrp As New ArrayList()

		Public HasRow As Boolean

		Public GrpIndex As Integer = 0

		Public RowIndex As Integer = 0

		' Page variables
		Public RecCount As Integer = 0 ' Record count

		Public StartGrp As Integer = 0 ' Start group

		Public StopGrp As Integer = 0	' Stop group

		Public TotalGrps As Integer = 0	' Total groups

		Public GrpCount As Integer = 0 ' Group count

		Public DisplayGrps As Integer = 20	' Groups per page

		Public GrpRange As Integer = 10

		Public Sort As String = ""

		Public Filter As String = ""

		Public UserIDFilter As String = ""

		' Clear field for ext filter
		Public ClearExtFilter As String = ""

		Public FilterApplied As Boolean

		Public ShowFirstHeader As Boolean

		Public Val As Object() = New Object(13) {}

		Public Cnt As Integer()() = New Integer(0)() {}

		Public Smry As Object()() = New Object(0)() {}

		Public Mn As Object()() = New Object(0)() {}

		Public Mx As Object()() = New Object(0)() {}

		Public GrandSmry As Object() = New Object(13) {}

		Public GrandMn As Object() = New Object(13) {}

		Public GrandMx As Object() = New Object(13) {}

		' Set up if accumulation required
		Public Col() As Boolean = {false, False, False, False, False, False, False, False, False, False, False, False, False, False}

		Public TotCount As Integer

		'
		' Page main
		'
		Public Sub Page_Main()

			' Aggregate variables		
			Dim nDtls As Integer = 14 ' No. of fields
			Dim nGrps As Integer = 1 ' No. of groups (level 0 used for grand total)
			For i As Integer = 0 To nGrps - 1
				Cnt(i) = New Integer(nDtls-1) {}
				Smry(i) = New Object(nDtls-1) {}
				Mn(i) = New Object(nDtls-1) {}
				Mx(i) = New Object(nDtls-1) {}
			Next

			' Set up groups per page dynamically
			SetUpDisplayGrps()

			' Load default filter values
			LoadDefaultFilters()

			' Set up popup filter
			SetupPopup()

			' Extended filter
			Dim sExtendedFilter As String = ""

			' Get dropdown values
			GetExtendedFilterValues()

			' Load custom filters
			Postergados.CustomFilters_Load()

			' Build extended filter
			sExtendedFilter = GetExtendedFilter()
			If (ew_NotEmpty(sExtendedFilter)) Then
				If (ew_NotEmpty(Filter)) Then
	  			Filter = "(" + Filter + ") AND (" + sExtendedFilter + ")"
				Else
					Filter = sExtendedFilter
				End If
			End If

			' Build popup filter
			Dim sPopupFilter As String = GetPopupFilter()

			'ew_SetDebugMsg("popup filter: " + sPopupFilter)
			If (ew_NotEmpty(sPopupFilter)) Then
				If (ew_NotEmpty(Filter)) Then
					Filter = "(" + Filter + ") AND (" + sPopupFilter + ")"
				Else
					Filter = sPopupFilter
				End If
			End If

			' Check if filter applied
			FilterApplied = CheckFilter()

			' Get sort
			Sort = GetSort()

			' Get total count
			Dim sSql As String = ewrpt_BuildReportSql(Postergados.SqlSelect, Postergados.SqlWhere, Postergados.SqlGroupBy, Postergados.SqlHaving, Postergados.SqlOrderBy, Filter, Sort)
			TotalGrps = GetCnt(sSql)
			If (DisplayGrps <= 0) Then ' Display all groups
				DisplayGrps = TotalGrps
			End If
			StartGrp = 1

			' Show header
			ShowFirstHeader = (TotalGrps > 0)

			'ShowFirstHeader = True ' Uncomment to always show header
			' Set up start position if not export all

			If (Postergados.ExportAll AndAlso ew_NotEmpty(Postergados.Export)) Then
				DisplayGrps = TotalGrps
			Else
				SetUpStartGroup()
			End If 

			' Get current page records
			dr = GetRs(sSql, StartGrp, DisplayGrps)
		End Sub

		' Accummulate summary
		Public Sub AccumulateSummary()
			Dim cntx As Integer = Smry.Length
			For ix As Integer = 0 To cntx - 1
				Dim cnty As Integer = Smry(ix).Length
				For iy As Integer = 1 To cnty - 1
					Cnt(ix)(iy) += 1
					If Col(iy) Then
						Dim valwrk As Object = Val(iy)
						If Convert.IsDBNull(valwrk) OrElse Not ewrpt_IsNumeric(valwrk) Then

							' Skip
						Else
							Smry(ix)(iy) = Convert.ToDouble(Smry(ix)(iy)) + Convert.ToDouble(valwrk)
							If Mn(ix)(iy) Is Nothing Then
								Mn(ix)(iy) = valwrk
								Mx(ix)(iy) = valwrk
							Else
								If Convert.ToDouble(Mn(ix)(iy)) > Convert.ToDouble(valwrk) Then
									Mn(ix)(iy) = valwrk
								End If
								If Convert.ToDouble(Mx(ix)(iy)) < Convert.ToDouble(valwrk) Then
									Mx(ix)(iy) = valwrk
								End If
							End If
						End If
					End If
				Next
			Next
			cntx = Smry.Length
			For ix As Integer = 1 To cntx - 1
				Cnt(ix)(0) += 1
			Next
		End Sub

		' Reset level summary
		Public Sub ResetLevelSummary(ByVal lvl As Integer)
			Dim cntx As Integer = Smry.Length
			For ix As Integer = lvl To cntx - 1
				Dim cnty As Integer = Smry(ix).Length
				For iy As Integer = 1 To cnty - 1
					Cnt(ix)(iy) = 0
					If Col(iy) Then
						Smry(ix)(iy) = 0
						Mn(ix)(iy) = Nothing
						Mx(ix)(iy) = Nothing
					End If
				Next
			Next
			cntx = Smry.Length
			For ix As Integer = lvl To cntx - 1
				Cnt(ix)(0) = 0
			Next

			' Reset record count
			RecCount = 0
		End Sub

		' Accummulate grand summary
		Public Sub AccumulateGrandSummary()
			Cnt(0)(0) += 1
			Dim cntgs As Integer = GrandSmry.Length
			For iy As Integer = 1 To cntgs - 1
				If Col(iy) Then
					Dim valwrk As Object = Val(iy)

							' skip
					If Convert.IsDBNull(valwrk) OrElse Not ewrpt_IsNumeric(valwrk) Then
					Else
						GrandSmry(iy) = Convert.ToDouble(GrandSmry(iy)) + Convert.ToDouble(valwrk)
						If ew_Empty(GrandMn(iy)) Then
							GrandMn(iy) = valwrk
							GrandMx(iy) = valwrk
						Else
							If Convert.ToDouble(GrandMn(iy)) > Convert.ToDouble(valwrk) Then
								GrandMn(iy) = valwrk
							End If
							If Convert.ToDouble(GrandMx(iy)) < Convert.ToDouble(valwrk) Then
								GrandMx(iy) = valwrk
							End If
						End If
					End If
				End If
			Next
		End Sub

		' Get count
		Public Function GetCnt(ByVal sql As String) As Integer
			Try
				Dim cnt As Object = ew_ExecuteScalar("SELECT COUNT(*) FROM (" + sql + ") AS EWRPT_TEMP_TABLE")
				If cnt IsNot Nothing Then
					Return ew_ConvertToInt(cnt)
				Else
					Throw New Exception("Failed to get record count")
				End If
			Catch
				Dim dr As SqlDataReader = Conn.GetTempDataReader(sql)
				Dim rscnt As Integer = 0
				If dr IsNot Nothing AndAlso dr.HasRows Then
					While dr.Read()
						rscnt += 1
					End While
				End If
				Conn.CloseTempDataReader()
				Return rscnt
			End Try
		End Function

		' Get rs
		Public Function GetRs(ByVal sql As String, ByVal start As Integer, ByVal grps As Integer) As SqlDataReader
			GrpIndex = -1
			Dim dr As SqlDataReader = Conn.GetDataReader(sql)
			For i As Integer = 1 To start - 1
				HasRow = dr.Read()
				GrpIndex += 1
				If Not HasRow Then
					Exit For
				End If
			Next
			Return dr
		End Function

		' Get row values
		Public Sub GetRow(opt As Integer)
			Dim Row As OrderedDictionary
			If (RowIndex >= rs.Count) Then Return
			RowIndex = IIf(opt = 1, 0, RowIndex + 1)
			If (RowIndex < rs.Count) Then
				Row = CType(rs(RowIndex), OrderedDictionary)		
				Postergados.Rut.DbValue = Row("Rut")
				Postergados.Paterno.DbValue = Row("Paterno")
				Postergados.Materno.DbValue = Row("Materno")
				Postergados.Nombre.DbValue = Row("Nombre")
				Postergados.Estado_Academico.DbValue = Row("Estado Academico")
				Postergados.Estado_Final.DbValue = Row("Estado Final")
				Postergados.AF1o.DbValue = Row("Año")
				Postergados.Periodo.DbValue = Row("Periodo")
				Postergados.Codigo_Carrera.DbValue = Row("Codigo Carrera")
				Postergados.Codigo.DbValue = Row("Codigo")
				Postergados.Descripcion.DbValue = Row("Descripcion")
				Postergados.Fecha_ResoluciF3n.DbValue = Row("Fecha Resolución")
				Postergados.ObservaciF3n.DbValue = Row("Observación")
				Val(1) = Postergados.Rut.CurrentValue
				Val(2) = Postergados.Nombre.CurrentValue
				Val(3) = Postergados.Paterno.CurrentValue
				Val(4) = Postergados.Materno.CurrentValue
				Val(5) = Postergados.Codigo_Carrera.CurrentValue
				Val(6) = Postergados.AF1o.CurrentValue
				Val(7) = Postergados.Periodo.CurrentValue
				Val(8) = Postergados.Codigo.CurrentValue
				Val(9) = Postergados.Estado_Academico.CurrentValue
				Val(10) = Postergados.Estado_Final.CurrentValue
				Val(11) = Postergados.Descripcion.CurrentValue
				Val(12) = Postergados.Fecha_ResoluciF3n.CurrentValue
				Val(13) = Postergados.ObservaciF3n.CurrentValue
			Else
				Postergados.Rut.DbValue = ""
				Postergados.Paterno.DbValue = ""
				Postergados.Materno.DbValue = ""
				Postergados.Nombre.DbValue = ""
				Postergados.Estado_Academico.DbValue = ""
				Postergados.Estado_Final.DbValue = ""
				Postergados.AF1o.DbValue = ""
				Postergados.Periodo.DbValue = ""
				Postergados.Codigo_Carrera.DbValue = ""
				Postergados.Codigo.DbValue = ""
				Postergados.Descripcion.DbValue = ""
				Postergados.Fecha_ResoluciF3n.DbValue = ""
				Postergados.ObservaciF3n.DbValue = ""
			End If
		End Sub

		' Get row values from data reader ' ASPXRPT
		Public Function GetRow() As Boolean
			HasRow = (dr IsNot Nothing AndAlso dr.Read()) 
			If (HasRow) Then
				GrpIndex += 1			
				Postergados.Rut.DbValue = dr("Rut")
				Postergados.Paterno.DbValue = dr("Paterno")
				Postergados.Materno.DbValue = dr("Materno")
				Postergados.Nombre.DbValue = dr("Nombre")
				Postergados.Estado_Academico.DbValue = dr("Estado Academico")
				Postergados.Estado_Final.DbValue = dr("Estado Final")
				Postergados.AF1o.DbValue = dr("Año")
				Postergados.Periodo.DbValue = dr("Periodo")
				Postergados.Codigo_Carrera.DbValue = dr("Codigo Carrera")
				Postergados.Codigo.DbValue = dr("Codigo")
				Postergados.Descripcion.DbValue = dr("Descripcion")
				Postergados.Fecha_ResoluciF3n.DbValue = dr("Fecha Resolución")
				Postergados.ObservaciF3n.DbValue = dr("Observación")
				Val(1) = Postergados.Rut.CurrentValue
				Val(2) = Postergados.Nombre.CurrentValue
				Val(3) = Postergados.Paterno.CurrentValue
				Val(4) = Postergados.Materno.CurrentValue
				Val(5) = Postergados.Codigo_Carrera.CurrentValue
				Val(6) = Postergados.AF1o.CurrentValue
				Val(7) = Postergados.Periodo.CurrentValue
				Val(8) = Postergados.Codigo.CurrentValue
				Val(9) = Postergados.Estado_Academico.CurrentValue
				Val(10) = Postergados.Estado_Final.CurrentValue
				Val(11) = Postergados.Descripcion.CurrentValue
				Val(12) = Postergados.Fecha_ResoluciF3n.CurrentValue
				Val(13) = Postergados.ObservaciF3n.CurrentValue
			Else				
				Postergados.Rut.DbValue = ""
				Postergados.Paterno.DbValue = ""
				Postergados.Materno.DbValue = ""
				Postergados.Nombre.DbValue = ""
				Postergados.Estado_Academico.DbValue = ""
				Postergados.Estado_Final.DbValue = ""
				Postergados.AF1o.DbValue = ""
				Postergados.Periodo.DbValue = ""
				Postergados.Codigo_Carrera.DbValue = ""
				Postergados.Codigo.DbValue = ""
				Postergados.Descripcion.DbValue = ""
				Postergados.Fecha_ResoluciF3n.DbValue = ""
				Postergados.ObservaciF3n.DbValue = ""
			End If
			Return HasRow		
		End Function

		' Set up starting group
		Public Sub SetUpStartGroup()

			' Exit if no groups
			If DisplayGrps = 0 Then
				Return
			End If

			' Check for a "start" parameter 
			If ew_NotEmpty(ew_Get(EWRPT_TABLE_START_GROUP)) AndAlso ewrpt_IsNumeric(ew_Get(EWRPT_TABLE_START_GROUP)) Then
				StartGrp = ew_ConvertToInt(ew_Get(EWRPT_TABLE_START_GROUP))
				Postergados.StartGroup = StartGrp
			ElseIf ew_NotEmpty(ew_Get("pageno")) Then
				If ewrpt_IsNumeric(ew_Get("pageno")) Then
					Dim nPageNo As Integer = ew_ConvertToInt(ew_Get("pageno"))
					StartGrp = (nPageNo - 1) * DisplayGrps + 1
					If StartGrp <= 0 Then
						StartGrp = 1
					ElseIf StartGrp >= ((TotalGrps - 1) / DisplayGrps) * DisplayGrps + 1 Then
						StartGrp = ((TotalGrps - 1) / DisplayGrps) * DisplayGrps + 1
					End If
					Postergados.StartGroup = StartGrp
				Else
					StartGrp = Postergados.StartGroup
				End If
			Else
				StartGrp = Postergados.StartGroup
			End If

			' Check if correct start group counter 
			If StartGrp <= 0 Then	' Avoid invalid start group counter 
				StartGrp = 1 ' Reset start group counter 
				Postergados.StartGroup = StartGrp
			ElseIf StartGrp > TotalGrps Then ' Avoid starting group > total groups 
				StartGrp = ((TotalGrps - 1) / DisplayGrps) * DisplayGrps + 1 ' Point to last page first group 
				Postergados.StartGroup = StartGrp
			ElseIf (StartGrp - 1) Mod DisplayGrps <> 0 Then
				StartGrp = ((StartGrp - 1) / DisplayGrps) * DisplayGrps + 1	' Point to page boundary 
				Postergados.StartGroup = StartGrp
			End If
		End Sub

		' Set up popup
		Public Sub SetupPopup()	
		  Dim sSql As String = ""
		  Dim rswrk As SqlDataReader
		  Dim bNullValue As Boolean
		  Dim bEmptyValue As Boolean
			Dim grpval As Object

			' Initialize popup
			' Process post back form

			If (HttpContext.Current.Request.Form.Count > 0) Then
				Dim sName As String = ew_Post("popup") ' Get popup form name
				If (ew_NotEmpty(sName)) Then
					Dim arValues As Object = HttpContext.Current.Request.Form.GetValues("sel_" + sName)
					Dim cntValues As Integer = CType(arValues, String()).Length
					If (cntValues > 0) Then
						If (ew_Empty(CType(arValues, String())(0))) Then ' Select all
							arValues = EWRPT_INIT_VALUE
						End If
						If Not ewrpt_MatchedArray(arValues, ew_Session("sel_" + sName)) Then
							If (HasSessionFilterValues(sName)) Then
								ClearExtFilter = sName ' Clear extended filter for this field
							End If
						End If
						ew_Session("sel_" + sName) = arValues
						ew_Session("rf_" + sName) = ew_Post("rf_" + sName)
						ew_Session("rt_" + sName) = ew_Post("rt_" + sName)
						ResetPager()
					End If
				End If

			' Get 'reset' command
			ElseIf (ew_NotEmpty(ew_Get("cmd"))) Then '  Get reset cmd
				Dim sCmd As String = ew_Get("cmd")
				If (ew_SameText(sCmd, "reset")) Then
					ResetPager()
				End If
			End If

			' Load selection criteria to array
		End Sub

		' Reset pager to starting position
		Public Sub ResetPager()
			StartGrp = 1
			Postergados.StartGroup = StartGrp
		End Sub

		Public Pager As cPrevNextPager

		' Set up number of groups displayed per page based on form element GrpPerPage 
		Public Sub SetUpDisplayGrps()
			Dim sWrk As String = ew_Get(EWRPT_TABLE_GROUP_PER_PAGE)
			If (ew_NotEmpty(sWrk)) Then 
				If (ewrpt_IsNumeric(sWrk)) Then 
					DisplayGrps = ew_ConvertToInt(sWrk) 
				Else 
					If (ew_SameText(sWrk, "ALL")) Then ' Display all records 
						DisplayGrps = -1 
					Else 
						DisplayGrps = 20 ' Non-numeric, load default 
					End If 
				End If				
				Postergados.GroupPerPage = DisplayGrps ' Save to session
				StartGrp = 1 ' Reset start position (reset command)				
				Postergados.StartGroup = StartGrp
			Else				
				If (Postergados.GroupPerPage <> 0) Then 
					DisplayGrps = ew_ConvertToInt(Postergados.GroupPerPage) ' Restore from Session 
				Else 
					DisplayGrps = 20 ' Load default 
				End If 
			End If 
		End Sub 

		Public Sub RenderRow()		
			If (Postergados.RowTotalType = EWRPT_ROWTOTAL_GRAND) Then ' Grand total

				' Get total count from SQL directly
				Dim sSql As String = ewrpt_BuildReportSql(Postergados.SqlSelectCount, Postergados.SqlWhere, Postergados.SqlGroupBy, Postergados.SqlHaving, "", Filter, "")
				TotCount = ew_ConvertToInt(ew_ExecuteScalar(sSql))				
			End If

			' Call Row_Rendering event
			Postergados.Row_Rendering()

			'
			' Render view codes
			'

			If (Postergados.RowType = EWRPT_ROWTYPE_TOTAL) Then ' Summary row

				' Rut
				Postergados.Rut.ViewValue = Convert.ToString(Postergados.Rut.Summary)

				' Nombre
				Postergados.Nombre.ViewValue = Convert.ToString(Postergados.Nombre.Summary)

				' Paterno
				Postergados.Paterno.ViewValue = Convert.ToString(Postergados.Paterno.Summary)

				' Materno
				Postergados.Materno.ViewValue = Convert.ToString(Postergados.Materno.Summary)

				' Codigo Carrera
				Postergados.Codigo_Carrera.ViewValue = Convert.ToString(Postergados.Codigo_Carrera.Summary)

				' Año
				Postergados.AF1o.ViewValue = Convert.ToString(Postergados.AF1o.Summary)

				' Periodo
				Postergados.Periodo.ViewValue = Convert.ToString(Postergados.Periodo.Summary)

				' Codigo
				Postergados.Codigo.ViewValue = Convert.ToString(Postergados.Codigo.Summary)

				' Estado Academico
				Postergados.Estado_Academico.ViewValue = Convert.ToString(Postergados.Estado_Academico.Summary)

				' Estado Final
				Postergados.Estado_Final.ViewValue = Convert.ToString(Postergados.Estado_Final.Summary)

				' Descripcion
				Postergados.Descripcion.ViewValue = Convert.ToString(Postergados.Descripcion.Summary)

				' Fecha Resolución
				Postergados.Fecha_ResoluciF3n.ViewValue = Convert.ToString(Postergados.Fecha_ResoluciF3n.Summary)
				Postergados.Fecha_ResoluciF3n.ViewValue = ew_FormatDateTime(Postergados.Fecha_ResoluciF3n.ViewValue, 7)

				' Observación
				Postergados.ObservaciF3n.ViewValue = Convert.ToString(Postergados.ObservaciF3n.Summary)
			Else

				' Rut
				Postergados.Rut.ViewValue = Convert.ToString(Postergados.Rut.CurrentValue)
				Postergados.Rut.CellAttrs("class") = IIf(RecCount Mod 2 <> 1, "ewTableAltRow", "ewTableRow")

				' Nombre
				Postergados.Nombre.ViewValue = Convert.ToString(Postergados.Nombre.CurrentValue)
				Postergados.Nombre.CellAttrs("class") = IIf(RecCount Mod 2 <> 1, "ewTableAltRow", "ewTableRow")

				' Paterno
				Postergados.Paterno.ViewValue = Convert.ToString(Postergados.Paterno.CurrentValue)
				Postergados.Paterno.CellAttrs("class") = IIf(RecCount Mod 2 <> 1, "ewTableAltRow", "ewTableRow")

				' Materno
				Postergados.Materno.ViewValue = Convert.ToString(Postergados.Materno.CurrentValue)
				Postergados.Materno.CellAttrs("class") = IIf(RecCount Mod 2 <> 1, "ewTableAltRow", "ewTableRow")

				' Codigo Carrera
				Postergados.Codigo_Carrera.ViewValue = Convert.ToString(Postergados.Codigo_Carrera.CurrentValue)
				Postergados.Codigo_Carrera.CellAttrs("class") = IIf(RecCount Mod 2 <> 1, "ewTableAltRow", "ewTableRow")

				' Año
				Postergados.AF1o.ViewValue = Convert.ToString(Postergados.AF1o.CurrentValue)
				Postergados.AF1o.CellAttrs("class") = IIf(RecCount Mod 2 <> 1, "ewTableAltRow", "ewTableRow")

				' Periodo
				Postergados.Periodo.ViewValue = Convert.ToString(Postergados.Periodo.CurrentValue)
				Postergados.Periodo.CellAttrs("class") = IIf(RecCount Mod 2 <> 1, "ewTableAltRow", "ewTableRow")

				' Codigo
				Postergados.Codigo.ViewValue = Convert.ToString(Postergados.Codigo.CurrentValue)
				Postergados.Codigo.CellAttrs("class") = IIf(RecCount Mod 2 <> 1, "ewTableAltRow", "ewTableRow")

				' Estado Academico
				Postergados.Estado_Academico.ViewValue = Convert.ToString(Postergados.Estado_Academico.CurrentValue)
				Postergados.Estado_Academico.CellAttrs("class") = IIf(RecCount Mod 2 <> 1, "ewTableAltRow", "ewTableRow")

				' Estado Final
				Postergados.Estado_Final.ViewValue = Convert.ToString(Postergados.Estado_Final.CurrentValue)
				Postergados.Estado_Final.CellAttrs("class") = IIf(RecCount Mod 2 <> 1, "ewTableAltRow", "ewTableRow")

				' Descripcion
				Postergados.Descripcion.ViewValue = Convert.ToString(Postergados.Descripcion.CurrentValue)
				Postergados.Descripcion.CellAttrs("class") = IIf(RecCount Mod 2 <> 1, "ewTableAltRow", "ewTableRow")

				' Fecha Resolución
				Postergados.Fecha_ResoluciF3n.ViewValue = Convert.ToString(Postergados.Fecha_ResoluciF3n.CurrentValue)
				Postergados.Fecha_ResoluciF3n.ViewValue = ew_FormatDateTime(Postergados.Fecha_ResoluciF3n.ViewValue, 7)
				Postergados.Fecha_ResoluciF3n.CellAttrs("class") = IIf(RecCount Mod 2 <> 1, "ewTableAltRow", "ewTableRow")

				' Observación
				Postergados.ObservaciF3n.ViewValue = Convert.ToString(Postergados.ObservaciF3n.CurrentValue)
				Postergados.ObservaciF3n.CellAttrs("class") = IIf(RecCount Mod 2 <> 1, "ewTableAltRow", "ewTableRow")
			End If

			' Rut
			Postergados.Rut.HrefValue = ""

			' Nombre
			Postergados.Nombre.HrefValue = ""

			' Paterno
			Postergados.Paterno.HrefValue = ""

			' Materno
			Postergados.Materno.HrefValue = ""

			' Codigo Carrera
			Postergados.Codigo_Carrera.HrefValue = ""

			' Año
			Postergados.AF1o.HrefValue = ""

			' Periodo
			Postergados.Periodo.HrefValue = ""

			' Codigo
			Postergados.Codigo.HrefValue = ""

			' Estado Academico
			Postergados.Estado_Academico.HrefValue = ""

			' Estado Final
			Postergados.Estado_Final.HrefValue = ""

			' Descripcion
			Postergados.Descripcion.HrefValue = ""

			' Fecha Resolución
			Postergados.Fecha_ResoluciF3n.HrefValue = ""

			' Observación
			Postergados.ObservaciF3n.HrefValue = ""

			' Call Row_Rendered event
			Postergados.Row_Rendered()
		End Sub

	'
	' Extended filter
	'
	' Get extended filter values
	Public Sub GetExtendedFilterValues()
		Dim sSelect As String
		Dim sWhere As String = ""
		Dim sOrderBy As String
		Dim wrkSql As String

			' Field Codigo Carrera
			sSelect = "SELECT DISTINCT [Codigo Carrera] FROM " + Postergados.SqlFrom
			sOrderBy = "[Codigo Carrera] ASC"
			wrkSql = ewrpt_BuildReportSql(sSelect, Postergados.SqlWhere, "", "", sOrderBy, UserIDFilter, "")
			Postergados.Codigo_Carrera.DropDownList = ParentPage.ewrpt_GetDistinctValues("", wrkSql)

			' Field Año
			sSelect = "SELECT DISTINCT [Año] FROM " + Postergados.SqlFrom
			sOrderBy = "[Año] ASC"
			wrkSql = ewrpt_BuildReportSql(sSelect, Postergados.SqlWhere, "", "", sOrderBy, UserIDFilter, "")
			Postergados.AF1o.DropDownList = ParentPage.ewrpt_GetDistinctValues("", wrkSql)

			' Field Periodo
			sSelect = "SELECT DISTINCT [Periodo] FROM " + Postergados.SqlFrom
			sOrderBy = "[Periodo] ASC"
			wrkSql = ewrpt_BuildReportSql(sSelect, Postergados.SqlWhere, "", "", sOrderBy, UserIDFilter, "")
			Postergados.Periodo.DropDownList = ParentPage.ewrpt_GetDistinctValues("", wrkSql)

			' Field Codigo
			sSelect = "SELECT DISTINCT [Codigo] FROM " + Postergados.SqlFrom
			sOrderBy = "[Codigo] ASC"
			wrkSql = ewrpt_BuildReportSql(sSelect, Postergados.SqlWhere, "", "", sOrderBy, UserIDFilter, "")
			Postergados.Codigo.DropDownList = ParentPage.ewrpt_GetDistinctValues("", wrkSql)

			' Field Estado Academico
			sSelect = "SELECT DISTINCT [Estado Academico] FROM " + Postergados.SqlFrom
			sOrderBy = "[Estado Academico] ASC"
			wrkSql = ewrpt_BuildReportSql(sSelect, Postergados.SqlWhere, "", "", sOrderBy, UserIDFilter, "")
			Postergados.Estado_Academico.DropDownList = ParentPage.ewrpt_GetDistinctValues("", wrkSql)

			' Field Estado Final
			sSelect = "SELECT DISTINCT [Estado Final] FROM " + Postergados.SqlFrom
			sOrderBy = "[Estado Final] ASC"
			wrkSql = ewrpt_BuildReportSql(sSelect, Postergados.SqlWhere, "", "", sOrderBy, UserIDFilter, "")
			Postergados.Estado_Final.DropDownList = ParentPage.ewrpt_GetDistinctValues("", wrkSql)
	End Sub

	' Return extended filter
	Public Function GetExtendedFilter() As String
		Dim sFilter As String = ""
	  Dim sWrk As String
	  Dim bPostBack As Boolean = (HttpContext.Current.Request.Form.Count > 0)
	  Dim bRestoreSession As Boolean = True
		Dim bSetupFilter As Boolean = False		
		If bPostBack Then ' Reset extended filter if filter changed
		ElseIf ew_SameText(ew_Get("cmd"), "reset") Then ' Reset search command

				' Field Rut
				SetSessionFilterValues(Postergados.Rut)

				' Field Nombre
				SetSessionFilterValues(Postergados.Nombre)

				' Field Paterno
				SetSessionFilterValues(Postergados.Paterno)

				' Field Materno
				SetSessionFilterValues(Postergados.Materno)

				' Field Codigo Carrera
				SetSessionDropDownValue(Postergados.Codigo_Carrera.DropDownValue, Postergados.Codigo_Carrera)

				' Field Año
				SetSessionDropDownValue(Postergados.AF1o.DropDownValue, Postergados.AF1o)

				' Field Periodo
				SetSessionDropDownValue(Postergados.Periodo.DropDownValue, Postergados.Periodo)

				' Field Codigo
				SetSessionDropDownValue(Postergados.Codigo.DropDownValue, Postergados.Codigo)

				' Field Estado Academico
				SetSessionDropDownValue(Postergados.Estado_Academico.DropDownValue, Postergados.Estado_Academico)

				' Field Estado Final
				SetSessionDropDownValue(Postergados.Estado_Final.DropDownValue, Postergados.Estado_Final)
			bSetupFilter = True ' Set up filter required
		Else

				' Field Rut
				If (GetFilterValues(Postergados.Rut)) Then
					bSetupFilter = True ' Set up filter required
					bRestoreSession = False ' Do not restore from session
				End If

				' Field Nombre
				If (GetFilterValues(Postergados.Nombre)) Then
					bSetupFilter = True ' Set up filter required
					bRestoreSession = False ' Do not restore from session
				End If

				' Field Paterno
				If (GetFilterValues(Postergados.Paterno)) Then
					bSetupFilter = True ' Set up filter required
					bRestoreSession = False ' Do not restore from session
				End If

				' Field Materno
				If (GetFilterValues(Postergados.Materno)) Then
					bSetupFilter = True ' Set up filter required
					bRestoreSession = False ' Do not restore from session
				End If

				' Field Codigo Carrera
				If (GetDropDownValue(Postergados.Codigo_Carrera)) Then
					bSetupFilter = True ' Set up filter required
					bRestoreSession = False ' Do not restore from session
				ElseIf Not ew_IsArrayList(Postergados.Codigo_Carrera.DropDownValue) Then
					If Not ew_SameStr(Postergados.Codigo_Carrera.DropDownValue, EWRPT_INIT_VALUE) AndAlso ew_Session("sv_Postergados_Codigo_Carrera") Is Nothing Then
						bSetupFilter = True ' Set up filter required
					End If
				End If

				' Field Año
				If (GetDropDownValue(Postergados.AF1o)) Then
					bSetupFilter = True ' Set up filter required
					bRestoreSession = False ' Do not restore from session
				ElseIf Not ew_IsArrayList(Postergados.AF1o.DropDownValue) Then
					If Not ew_SameStr(Postergados.AF1o.DropDownValue, EWRPT_INIT_VALUE) AndAlso ew_Session("sv_Postergados_AF1o") Is Nothing Then
						bSetupFilter = True ' Set up filter required
					End If
				End If

				' Field Periodo
				If (GetDropDownValue(Postergados.Periodo)) Then
					bSetupFilter = True ' Set up filter required
					bRestoreSession = False ' Do not restore from session
				ElseIf Not ew_IsArrayList(Postergados.Periodo.DropDownValue) Then
					If Not ew_SameStr(Postergados.Periodo.DropDownValue, EWRPT_INIT_VALUE) AndAlso ew_Session("sv_Postergados_Periodo") Is Nothing Then
						bSetupFilter = True ' Set up filter required
					End If
				End If

				' Field Codigo
				If (GetDropDownValue(Postergados.Codigo)) Then
					bSetupFilter = True ' Set up filter required
					bRestoreSession = False ' Do not restore from session
				ElseIf Not ew_IsArrayList(Postergados.Codigo.DropDownValue) Then
					If Not ew_SameStr(Postergados.Codigo.DropDownValue, EWRPT_INIT_VALUE) AndAlso ew_Session("sv_Postergados_Codigo") Is Nothing Then
						bSetupFilter = True ' Set up filter required
					End If
				End If

				' Field Estado Academico
				If (GetDropDownValue(Postergados.Estado_Academico)) Then
					bSetupFilter = True ' Set up filter required
					bRestoreSession = False ' Do not restore from session
				ElseIf Not ew_IsArrayList(Postergados.Estado_Academico.DropDownValue) Then
					If Not ew_SameStr(Postergados.Estado_Academico.DropDownValue, EWRPT_INIT_VALUE) AndAlso ew_Session("sv_Postergados_Estado_Academico") Is Nothing Then
						bSetupFilter = True ' Set up filter required
					End If
				End If

				' Field Estado Final
				If (GetDropDownValue(Postergados.Estado_Final)) Then
					bSetupFilter = True ' Set up filter required
					bRestoreSession = False ' Do not restore from session
				ElseIf Not ew_IsArrayList(Postergados.Estado_Final.DropDownValue) Then
					If Not ew_SameStr(Postergados.Estado_Final.DropDownValue, EWRPT_INIT_VALUE) AndAlso ew_Session("sv_Postergados_Estado_Final") Is Nothing Then
						bSetupFilter = True ' Set up filter required
					End If
				End If
			If Not ValidateForm() Then
				Message = gsFormError
				Return sFilter
			End If
		End If

		' Restore session
		If bRestoreSession Then

			' Field Rut
			GetSessionFilterValues(Postergados.Rut)

			' Field Nombre
			GetSessionFilterValues(Postergados.Nombre)

			' Field Paterno
			GetSessionFilterValues(Postergados.Paterno)

			' Field Materno
			GetSessionFilterValues(Postergados.Materno)

			' Field Codigo Carrera
			GetSessionDropDownValue(Postergados.Codigo_Carrera)

			' Field Año
			GetSessionDropDownValue(Postergados.AF1o)

			' Field Periodo
			GetSessionDropDownValue(Postergados.Periodo)

			' Field Codigo
			GetSessionDropDownValue(Postergados.Codigo)

			' Field Estado Academico
			GetSessionDropDownValue(Postergados.Estado_Academico)

			' Field Estado Final
			GetSessionDropDownValue(Postergados.Estado_Final)
		End If

		' Call page filter validated event
		Postergados.Page_FilterValidated()

		' Build SQL
		' Field Rut

		BuildExtendedFilter(Postergados.Rut, sFilter)

		' Field Nombre
		BuildExtendedFilter(Postergados.Nombre, sFilter)

		' Field Paterno
		BuildExtendedFilter(Postergados.Paterno, sFilter)

		' Field Materno
		BuildExtendedFilter(Postergados.Materno, sFilter)

		' Field Codigo Carrera
		BuildDropDownFilter(Postergados.Codigo_Carrera, sFilter, "")

		' Field Año
		BuildDropDownFilter(Postergados.AF1o, sFilter, "")

		' Field Periodo
		BuildDropDownFilter(Postergados.Periodo, sFilter, "")

		' Field Codigo
		BuildDropDownFilter(Postergados.Codigo, sFilter, "")

		' Field Estado Academico
		BuildDropDownFilter(Postergados.Estado_Academico, sFilter, "")

		' Field Estado Final
		BuildDropDownFilter(Postergados.Estado_Final, sFilter, "")

		' Save parms to Session
		' Field Rut

		SetSessionFilterValues(Postergados.Rut)

		' Field Nombre
		SetSessionFilterValues(Postergados.Nombre)

		' Field Paterno
		SetSessionFilterValues(Postergados.Paterno)

		' Field Materno
		SetSessionFilterValues(Postergados.Materno)

		' Field Codigo Carrera
		SetSessionDropDownValue(Postergados.Codigo_Carrera.DropDownValue, Postergados.Codigo_Carrera)

		' Field Año
		SetSessionDropDownValue(Postergados.AF1o.DropDownValue, Postergados.AF1o)

		' Field Periodo
		SetSessionDropDownValue(Postergados.Periodo.DropDownValue, Postergados.Periodo)

		' Field Codigo
		SetSessionDropDownValue(Postergados.Codigo.DropDownValue, Postergados.Codigo)

		' Field Estado Academico
		SetSessionDropDownValue(Postergados.Estado_Academico.DropDownValue, Postergados.Estado_Academico)

		' Field Estado Final
		SetSessionDropDownValue(Postergados.Estado_Final.DropDownValue, Postergados.Estado_Final)

		' Setup filter
		If bSetupFilter Then
			Dim sSql As String
		End If
		Return sFilter
	End Function	

	' Get drop down value from querystring 
	Public Function GetDropDownValue(ByRef fld As crField) As Boolean
		If HttpContext.Current.Request.Form.Count > 0 Then
			Return False
		End If

		' Skip post back
		Dim parm As String = fld.FldVar.Substring(2)
		If ew_NotEmpty(ew_Get("sv_" + parm)) Then
			Dim sv As Object = HttpContext.Current.Request.QueryString.GetValues("sv_" + parm)
			If DirectCast(sv, String()).Length = 1 Then
				sv = ew_Get("sv_" + parm)
			End If
			fld.DropDownValue = sv
			Return True
		End If
		Return False
	End Function

	' Get filter values from querystring 
	Public Function GetFilterValues(ByRef fld As crField) As Boolean
		Dim parm As String = fld.FldVar.Substring(2)
		Dim ReturnValue As Boolean = False

		' Skip post back 
		If HttpContext.Current.Request.Form.Count > 0 Then	Return ReturnValue
		If HttpContext.Current.Request.QueryString("sv1_" & parm) IsNot Nothing Then
			fld.SearchValue = ew_Get("sv1_" & parm)
			ReturnValue = True
		End If
		If HttpContext.Current.Request.QueryString("so1_" & parm) IsNot Nothing Then
			fld.SearchOperator = ew_Get("so1_" & parm)
			ReturnValue = True
		End If
		If HttpContext.Current.Request.QueryString("sc_" & parm) IsNot Nothing Then
			fld.SearchCondition = ew_Get("sc_" & parm)
			ReturnValue = True
		End If
		If HttpContext.Current.Request.QueryString("sv2_" & parm) IsNot Nothing Then
			fld.SearchValue2 = ew_Get("sv2_" & parm)
			ReturnValue = True
		End If
		If HttpContext.Current.Request.QueryString("so2_" & parm) IsNot Nothing Then
			fld.SearchOperator2 = ew_Get("so2_" & parm)
			ReturnValue = True
		End If
		Return ReturnValue
	End Function

			' Set default ext filter
	Public Sub SetDefaultExtFilter(ByRef fld As crField, ByVal so1 As String, ByVal sv1 As String, ByVal sc As String, ByVal so2 As String, ByVal sv2 As String)
		fld.DefaultSearchValue = sv1 ' Default ext filter value 1
		fld.DefaultSearchOperator = so1	' Default search operator 1
		fld.DefaultSearchCondition = sc	' Default search condition (if operator 2 is enabled)
		fld.DefaultSearchValue2 = sv2	' Default ext filter value 2 (if operator 2 is enabled)
		fld.DefaultSearchOperator2 = so2 ' Default search operator 2 (if operator 2 is enabled)
	End Sub

	' Apply default ext filter
	Public Sub ApplyDefaultExtFilter(ByRef fld As crField)
		fld.SearchValue = fld.DefaultSearchValue
		fld.SearchOperator = fld.DefaultSearchOperator
		fld.SearchCondition = fld.DefaultSearchCondition
		fld.SearchValue2 = fld.DefaultSearchValue2
		fld.SearchOperator2 = fld.DefaultSearchOperator2
	End Sub

	' Check if Text Filter applied
	Public Function TextFilterApplied(ByRef fld As crField) As Boolean
		Return (Not ew_SameStr(fld.SearchValue, fld.DefaultSearchValue) OrElse Not ew_SameStr(fld.SearchValue2, fld.DefaultSearchValue2) OrElse (ew_NotEmpty(fld.SearchValue) AndAlso Not ew_SameStr(fld.SearchOperator, fld.DefaultSearchOperator)) OrElse (ew_NotEmpty(fld.SearchValue2) AndAlso Not ew_SameStr(fld.SearchOperator2, fld.DefaultSearchOperator2)) OrElse Not ew_SameStr(fld.SearchCondition, fld.DefaultSearchCondition))
	End Function

	' Check if Non-Text Filter applied
	Public Function NonTextFilterApplied(ByRef fld As crField) As Boolean
		If ewrpt_IsArray(fld.DefaultDropDownValue) AndAlso ewrpt_IsArray(fld.DropDownValue) Then
			Dim ar1 As String() = DirectCast(fld.DropDownValue, String())
			Dim ar2 As String() = DirectCast(fld.DefaultDropDownValue, String())
			If ar1.Length <> ar2.Length Then
				Return True
			Else
				Array.Sort(ar1)
				Array.Sort(ar2)
				Return Not ewrpt_MatchedArray(ar1, ar2)
			End If
		ElseIf ewrpt_IsArray(fld.DefaultDropDownValue) OrElse ewrpt_IsArray(fld.DropDownValue) Then
			Return True
		Else
			Dim v1 As String = Convert.ToString(fld.DefaultDropDownValue)
			If v1 = EWRPT_INIT_VALUE Then
				v1 = ""
			End If
			Dim v2 As String = Convert.ToString(fld.DropDownValue)
			If v2 = EWRPT_INIT_VALUE OrElse v2 = EWRPT_ALL_VALUE Then
				v2 = ""
			End If
			Return (Not ew_SameStr(v1, v2))
		End If
	End Function

	' Load selection from a filter clause
	Public Sub LoadSelectionFromFilter(ByRef fld As crField, ByVal filter As String, ByRef sel As ArrayList)
		sel.Clear()
		If ew_NotEmpty(filter) Then
			Dim sSql As String = ewrpt_BuildReportSql(fld.SqlSelect, "", "", "", fld.SqlOrderBy, filter, "")
			ParentPage.ewrpt_LoadArrayListFromSql(sSql, sel)			
		End If
	End Sub

	' Load selection from a filter clause
	Public Sub LoadSelectionFromFilter(ByRef fld As crField, ByVal filter As String, ByRef sel As String())
		sel = New String(-1) {}
		If ew_NotEmpty(filter) Then
			Dim sSql As String = ewrpt_BuildReportSql(fld.SqlSelect, "", "", "", fld.SqlOrderBy, filter, "")
			ParentPage.ewrpt_LoadArrayFromSql(sSql, sel)
		End If
	End Sub

	' Get dropdown value from Session 
	Public Sub GetSessionDropDownValue(ByRef fld As crField)
		Dim parm As String = fld.FldVar.Substring(2)
		If ew_Session("sv_Postergados_" + parm) IsNot Nothing Then
			fld.DropDownValue = ew_Session("sv_Postergados_" + parm)
		End If
	End Sub

	' Set dropdown value to Session 
	Public Sub SetSessionDropDownValue(ByVal sv As Object, ByRef fld As crField)
		Dim parm As String = fld.FldVar.Substring(2)
		ew_Session("sv_Postergados_" + parm) = sv
	End Sub

	' Get filter values from Session 
	Public Sub GetSessionFilterValues(ByRef fld As crField)
		Dim parm As String = fld.FldVar.Substring(2)
		If ew_Session("sv1_Postergados_" + parm) IsNot Nothing Then
			fld.SearchValue = ew_Session("sv1_Postergados_" + parm)
		End If
		If ew_Session("so1_Postergados_" + parm) IsNot Nothing Then
			fld.SearchOperator = Convert.ToString(ew_Session("so1_Postergados_" + parm))
		End If
		If ew_Session("sc_Postergados_" + parm) IsNot Nothing Then
			fld.SearchCondition = Convert.ToString(ew_Session("sc_Postergados_" + parm))
		End If
		If ew_Session("sv2_Postergados_" + parm) IsNot Nothing Then
			fld.SearchValue2 = ew_Session("sv2_Postergados_" + parm)
		End If
		If ew_Session("so2_Postergados_" + parm) IsNot Nothing Then
			fld.SearchOperator2 = Convert.ToString(ew_Session("so2_Postergados_" + parm))
		End If
	End Sub

	' Set filter values to Session		
	Public Sub SetSessionFilterValues(ByRef fld As crField)
		Dim parm As String = fld.FldVar.Substring(2)
		ew_Session("sv1_Postergados_" + parm) = fld.SearchValue
		ew_Session("so1_Postergados_" + parm) = fld.SearchOperator
		ew_Session("sc_Postergados_" + parm) = fld.SearchCondition
		ew_Session("sv2_Postergados_" + parm) = fld.SearchValue2
		ew_Session("so2_Postergados_" + parm) = fld.SearchOperator2
	End Sub

	' Clear filter values from Session		
	Public Sub ClearSessionFilterValues(ByRef fld As crField)
		Dim parm As String = fld.FldVar.Substring(2)
		ew_Session("sv1_Postergados_" + parm) = ""
		ew_Session("so1_Postergados_" + parm) = "="
		ew_Session("sc_Postergados_" + parm) = "AND"
		ew_Session("sv2_Postergados_" + parm) = ""
		ew_Session("so2_Postergados_" + parm) = "="
	End Sub

	' Check if has Session filter values 
	Public Function HasSessionFilterValues(ByVal parm As String) As Boolean
		Return (ew_NotEmpty(ew_Session("sv_" + parm)) AndAlso Not ew_SameStr(ew_Session("sv_" + parm), EWRPT_INIT_VALUE)) OrElse (ew_NotEmpty(ew_Session("sv1_" + parm)) AndAlso Not ew_SameStr(ew_Session("sv1_" + parm), EWRPT_INIT_VALUE)) OrElse (ew_NotEmpty(ew_Session("sv2_" + parm)) AndAlso Not ew_SameStr(ew_Session("sv2_" + parm), EWRPT_INIT_VALUE))
	End Function

	' Check if dropdown filter
	Public Function DropDownFilterExist(ByRef fld As crField, ByVal FldOpr As String) As Boolean
		Dim sWrk As String = ""
		BuildDropDownFilter(fld, sWrk, FldOpr)
		Return ew_NotEmpty(sWrk)
	End Function

	' Build dropdown filter
	Public Sub BuildDropDownFilter(ByRef fld As crField, ByRef FilterClause As String, ByVal FldOpr As String)
		Dim FldVal As Object = fld.DropDownValue
		Dim sSql As String = ""
		If ewrpt_IsArray(FldVal) Then
			Dim arwrk As String() = DirectCast(FldVal, String())
			Dim sWrk As String
			For Each val As String In arwrk
				sWrk = GetDropDownFilter(fld, val, FldOpr)
				If ew_NotEmpty(sWrk) Then
					sSql = IIf(ew_NotEmpty(sSql), sSql + " OR " + sWrk, sWrk)
				End If
			Next
		Else
			sSql = GetDropDownFilter(fld, Convert.ToString(FldVal), FldOpr)
		End If
		If ew_NotEmpty(sSql) Then
			If ew_NotEmpty(FilterClause) Then
				FilterClause = "(" + FilterClause + ") AND "
			End If
			FilterClause += "(" + sSql + ")"
		End If
	End Sub

	' Get dropdown filter 
	Public Function GetDropDownFilter(ByRef fld As crField, ByVal FldVal As String, ByVal FldOpr As String) As String
		Dim FldName As String = fld.FldName
		Dim FldExpression As String = fld.FldExpression
		Dim FldDataType As Integer = fld.FldDataType
		Dim sWrk As String = ""
		If FldVal = EWRPT_NULL_VALUE Then
			sWrk = FldExpression + " IS NULL"
		ElseIf FldVal = EWRPT_EMPTY_VALUE Then
			sWrk = FldExpression + " = ''"
		Else
			If FldVal.StartsWith("@@") Then
				sWrk = ParentPage.ewrpt_GetCustomFilter(fld, FldVal)
			Else
				If ew_NotEmpty(FldVal) AndAlso Not ew_SameStr(FldVal, EWRPT_INIT_VALUE) AndAlso Not ew_SameStr(FldVal, EWRPT_ALL_VALUE) Then
					If FldDataType = EWRPT_DATATYPE_DATE AndAlso FldOpr <> "" Then
						sWrk = DateFilterString(FldOpr, FldVal, FldDataType)
					Else
						sWrk = FilterString("=", FldVal, FldDataType)
					End If
				End If
				If ew_NotEmpty(sWrk) Then
					sWrk = FldExpression + sWrk
				End If
			End If
		End If
		Return sWrk
	End Function

	' Check if extended filter
	Public Function ExtendedFilterExist(ByRef fld As crField) As Boolean
		Dim sExtWrk As String = ""
		BuildExtendedFilter(fld, sExtWrk)
		Return ew_NotEmpty(sExtWrk)
	End Function

	' Build extended filter 
	Public Sub BuildExtendedFilter(ByRef fld As crField, ByRef FilterClause As String)
		Dim IsValidValue As Boolean
		Dim sWrk As String = ""
		Dim wrkFldVal1 As String
		Dim wrkFldVal2 As String
		Dim FldName As String = fld.FldName
		Dim FldExpression As String = fld.FldExpression
		Dim FldDataType As Integer = fld.FldDataType
		Dim FldDateTimeFormat As Integer = fld.FldDateTimeFormat
		Dim FldVal1 As String = Convert.ToString(fld.SearchValue)
		Dim FldOpr1 As String = fld.SearchOperator
		Dim FldCond As String = fld.SearchCondition
		Dim FldVal2 As String = Convert.ToString(fld.SearchValue2)
		Dim FldOpr2 As String = fld.SearchOperator2
		FldOpr1 = IIf(FldOpr1 IsNot Nothing, FldOpr1.Trim().ToUpper(), "")
		If ew_Empty(FldOpr1) Then
			FldOpr1 = "="
		End If
		FldOpr2 = IIf(FldOpr2 IsNot Nothing, FldOpr2.Trim().ToUpper(), "")
		If ew_Empty(FldOpr2) Then
			FldOpr2 = "="
		End If
		wrkFldVal1 = FldVal1
		wrkFldVal2 = FldVal2
		If FldDataType = EWRPT_DATATYPE_BOOLEAN Then
			If wrkFldVal1 <> "" Then
				wrkFldVal1 = IIf(wrkFldVal1 = "1", "True", "False")
			End If
			If wrkFldVal2 <> "" Then
				wrkFldVal2 = IIf(wrkFldVal2 = "1", "True", "False")
			End If
		ElseIf FldDataType = EWRPT_DATATYPE_DATE Then
			If wrkFldVal1 <> "" Then
				wrkFldVal1 = ew_UnFormatDateTime(wrkFldVal1, FldDateTimeFormat)
			End If
			If wrkFldVal2 <> "" Then
				wrkFldVal2 = ew_UnFormatDateTime(wrkFldVal2, FldDateTimeFormat)
			End If
		End If
		If FldOpr1 = "BETWEEN" Then
			IsValidValue = (FldDataType <> EWRPT_DATATYPE_NUMBER) OrElse (FldDataType = EWRPT_DATATYPE_NUMBER AndAlso ewrpt_IsNumeric(wrkFldVal1) AndAlso ewrpt_IsNumeric(wrkFldVal2))
			If wrkFldVal1 <> "" AndAlso wrkFldVal2 <> "" AndAlso IsValidValue Then
				sWrk = FldExpression + " BETWEEN " + ew_QuotedValue(wrkFldVal1, FldDataType) + " AND " + ew_QuotedValue(wrkFldVal2, FldDataType)
			End If
		ElseIf FldOpr1 = "IS NULL" OrElse FldOpr1 = "IS NOT NULL" Then
			sWrk = FldExpression + " " + wrkFldVal1
		Else
			IsValidValue = (FldDataType <> EWRPT_DATATYPE_NUMBER) OrElse (FldDataType = EWRPT_DATATYPE_NUMBER AndAlso ewrpt_IsNumeric(wrkFldVal1))
			If wrkFldVal1 <> "" AndAlso IsValidValue AndAlso ew_IsValidOpr(FldOpr1, FldDataType) Then
				sWrk = FldExpression + FilterString(FldOpr1, wrkFldVal1, FldDataType)
			End If
			IsValidValue = (FldDataType <> EWRPT_DATATYPE_NUMBER) OrElse (FldDataType = EWRPT_DATATYPE_NUMBER AndAlso ewrpt_IsNumeric(wrkFldVal2))
			If wrkFldVal2 <> "" AndAlso IsValidValue AndAlso ew_IsValidOpr(FldOpr2, FldDataType) Then
				If sWrk <> "" Then
					sWrk += " " + IIf(FldCond = "OR", "OR", "AND") + " "
				End If
				sWrk += FldExpression + FilterString(FldOpr2, wrkFldVal2, FldDataType)
			End If
		End If
		If sWrk <> "" Then
			If FilterClause <> "" Then
				FilterClause += " AND "
			End If
			FilterClause += "(" + sWrk + ")"
		End If
	End Sub

	' Validate form
	Public Function ValidateForm() As Boolean

		' Initialize form error message
		gsFormError = ""

		' Check if validation required
		If Not EWRPT_SERVER_VALIDATE Then
			Return ew_Empty(gsFormError)
		End If

		' Return validate result
		Dim valid As Boolean = ew_Empty(gsFormError)

		' Call Form_CustomValidate event
		Dim sFormCustomError As String = ""
		valid = valid AndAlso Form_CustomValidate(sFormCustomError)
		If Not ew_Empty(sFormCustomError) Then
			gsFormError &= IIf(ew_NotEmpty(gsFormError), "<br>", "")
			gsFormError &= sFormCustomError
		End If
		Return valid
	End Function

	' Return filter string 
	Public Function FilterString(ByVal FldOpr As String, ByVal FldVal As Object, ByVal FldType As Integer) As String
		If ew_Empty(FldVal) Then
			Return ""
		End If
		If FldOpr = "LIKE" OrElse FldOpr = "NOT LIKE" Then
			Return " " + FldOpr + " " + ew_QuotedValue("%" + FldVal + "%", FldType)
		ElseIf FldOpr = "STARTS WITH" Then
			Return " LIKE " + ew_QuotedValue(FldVal + "%", FldType)
		Else
			Return " " + FldOpr + " " + ew_QuotedValue(FldVal, FldType)
		End If
	End Function

	' Return date search string 
	Public Function DateFilterString(ByVal FldOpr As String, ByVal FldVal As String, ByVal FldType As Integer) As String
		Dim wrkVal1 As Object = ewrpt_DateVal(FldOpr, FldVal, 1)
		Dim wrkVal2 As Object = ewrpt_DateVal(FldOpr, FldVal, 2)
		If ew_NotEmpty(wrkVal1) AndAlso ew_NotEmpty(wrkVal2) Then
			Return " BETWEEN " + ew_QuotedValue(wrkVal1, FldType) + " AND " + ew_QuotedValue(wrkVal2, FldType)
		Else
			Return ""
		End If
	End Function

	' Clear selection stored in session
	Public Sub ClearSessionSelection(parm As String)
		ew_Session("sel_Postergados_" & parm) = ""
		ew_Session("rf_Postergados_" & parm) = ""
		ew_Session("rt_Postergados_" & parm) = ""
	End Sub

	' Load selection from session
	Public Sub LoadSelectionFromSession(parm As String)
		Dim fld As crField = Postergados.Fields(parm)
		fld.SelectionList = CType(ew_Session("sel_Postergados_" & parm), String())
		fld.RangeFrom = Convert.ToString(ew_Session("rf_Postergados_" & parm))
		fld.RangeTo = Convert.ToString(ew_Session("rt_Postergados_" & parm))
	End Sub		

	' Load default value for filters
	Public Sub LoadDefaultFilters()	
		Dim sWrk As String
	  Dim sSql As String 

		' Set up default values for dropdown filters
			' Field Codigo Carrera

			Postergados.Codigo_Carrera.DefaultDropDownValue = EWRPT_INIT_VALUE
			Postergados.Codigo_Carrera.DropDownValue = Postergados.Codigo_Carrera.DefaultDropDownValue

			' Field Año
			Postergados.AF1o.DefaultDropDownValue = EWRPT_INIT_VALUE
			Postergados.AF1o.DropDownValue = Postergados.AF1o.DefaultDropDownValue

			' Field Periodo
			Postergados.Periodo.DefaultDropDownValue = EWRPT_INIT_VALUE
			Postergados.Periodo.DropDownValue = Postergados.Periodo.DefaultDropDownValue

			' Field Codigo
			Postergados.Codigo.DefaultDropDownValue = EWRPT_INIT_VALUE
			Postergados.Codigo.DropDownValue = Postergados.Codigo.DefaultDropDownValue

			' Field Estado Academico
			Postergados.Estado_Academico.DefaultDropDownValue = EWRPT_INIT_VALUE
			Postergados.Estado_Academico.DropDownValue = Postergados.Estado_Academico.DefaultDropDownValue

			' Field Estado Final
			Postergados.Estado_Final.DefaultDropDownValue = EWRPT_INIT_VALUE
			Postergados.Estado_Final.DropDownValue = Postergados.Estado_Final.DefaultDropDownValue

		' Set up default values for extended filters
			' Field Rut

			SetDefaultExtFilter(Postergados.Rut, "LIKE", Nothing, "AND", "=", Nothing)
			ApplyDefaultExtFilter(Postergados.Rut)	

			' Field Nombre
			SetDefaultExtFilter(Postergados.Nombre, "LIKE", Nothing, "AND", "=", Nothing)
			ApplyDefaultExtFilter(Postergados.Nombre)	

			' Field Paterno
			SetDefaultExtFilter(Postergados.Paterno, "LIKE", Nothing, "AND", "=", Nothing)
			ApplyDefaultExtFilter(Postergados.Paterno)	

			' Field Materno
			SetDefaultExtFilter(Postergados.Materno, "LIKE", Nothing, "AND", "=", Nothing)
			ApplyDefaultExtFilter(Postergados.Materno)	

		' Set up default values for popup filters
		' - NOTE: if extended filter is enabled, use default values in extended filter instead

	End Sub

	' Check if filter applied
	Public Function CheckFilter() As Boolean
	  Dim bFilterExist As Boolean = False

		' Check Rut extended filter
		If (TextFilterApplied(Postergados.Rut)) Then bFilterExist = True

		' Check Nombre extended filter
		If (TextFilterApplied(Postergados.Nombre)) Then bFilterExist = True

		' Check Paterno extended filter
		If (TextFilterApplied(Postergados.Paterno)) Then bFilterExist = True

		' Check Materno extended filter
		If (TextFilterApplied(Postergados.Materno)) Then bFilterExist = True

		' Check Codigo Carrera dropdown filter
		If (NonTextFilterApplied(Postergados.Codigo_Carrera)) Then bFilterExist = True

		' Check Año dropdown filter
		If (NonTextFilterApplied(Postergados.AF1o)) Then bFilterExist = True

		' Check Periodo dropdown filter
		If (NonTextFilterApplied(Postergados.Periodo)) Then bFilterExist = True

		' Check Codigo dropdown filter
		If (NonTextFilterApplied(Postergados.Codigo)) Then bFilterExist = True

		' Check Estado Academico dropdown filter
		If (NonTextFilterApplied(Postergados.Estado_Academico)) Then bFilterExist = True

		' Check Estado Final dropdown filter
		If (NonTextFilterApplied(Postergados.Estado_Final)) Then bFilterExist = True
		Return bFilterExist
	End Function	

	' Show list of filters
	Public Sub ShowFilterList()
		Dim sFilterList As String = ""
	  Dim sExtWrk As String
	  Dim sWrk As String

		' Field Rut
		sExtWrk = ""
		sWrk = ""
		BuildExtendedFilter(Postergados.Rut, sExtWrk)
		If (ew_NotEmpty(sExtWrk) OrElse ew_NotEmpty(sWrk)) Then sFilterList &= Postergados.Rut.FldCaption() & "<br>"
		If (ew_NotEmpty(sExtWrk)) Then sFilterList &= "&nbsp;&nbsp;" & sExtWrk & "<br>"
		If (ew_NotEmpty(sWrk)) Then sFilterList &= "&nbsp;&nbsp;" & sWrk & "<br>"

		' Field Nombre
		sExtWrk = ""
		sWrk = ""
		BuildExtendedFilter(Postergados.Nombre, sExtWrk)
		If (ew_NotEmpty(sExtWrk) OrElse ew_NotEmpty(sWrk)) Then sFilterList &= Postergados.Nombre.FldCaption() & "<br>"
		If (ew_NotEmpty(sExtWrk)) Then sFilterList &= "&nbsp;&nbsp;" & sExtWrk & "<br>"
		If (ew_NotEmpty(sWrk)) Then sFilterList &= "&nbsp;&nbsp;" & sWrk & "<br>"

		' Field Paterno
		sExtWrk = ""
		sWrk = ""
		BuildExtendedFilter(Postergados.Paterno, sExtWrk)
		If (ew_NotEmpty(sExtWrk) OrElse ew_NotEmpty(sWrk)) Then sFilterList &= Postergados.Paterno.FldCaption() & "<br>"
		If (ew_NotEmpty(sExtWrk)) Then sFilterList &= "&nbsp;&nbsp;" & sExtWrk & "<br>"
		If (ew_NotEmpty(sWrk)) Then sFilterList &= "&nbsp;&nbsp;" & sWrk & "<br>"

		' Field Materno
		sExtWrk = ""
		sWrk = ""
		BuildExtendedFilter(Postergados.Materno, sExtWrk)
		If (ew_NotEmpty(sExtWrk) OrElse ew_NotEmpty(sWrk)) Then sFilterList &= Postergados.Materno.FldCaption() & "<br>"
		If (ew_NotEmpty(sExtWrk)) Then sFilterList &= "&nbsp;&nbsp;" & sExtWrk & "<br>"
		If (ew_NotEmpty(sWrk)) Then sFilterList &= "&nbsp;&nbsp;" & sWrk & "<br>"

		' Field Codigo Carrera
		sExtWrk = ""
		sWrk = ""
		BuildDropDownFilter(Postergados.Codigo_Carrera, sExtWrk, "")
		If (ew_NotEmpty(sExtWrk) OrElse ew_NotEmpty(sWrk)) Then sFilterList &= Postergados.Codigo_Carrera.FldCaption() & "<br>"
		If (ew_NotEmpty(sExtWrk)) Then sFilterList &= "&nbsp;&nbsp;" & sExtWrk & "<br>"
		If (ew_NotEmpty(sWrk)) Then sFilterList &= "&nbsp;&nbsp;" & sWrk & "<br>"

		' Field Año
		sExtWrk = ""
		sWrk = ""
		BuildDropDownFilter(Postergados.AF1o, sExtWrk, "")
		If (ew_NotEmpty(sExtWrk) OrElse ew_NotEmpty(sWrk)) Then sFilterList &= Postergados.AF1o.FldCaption() & "<br>"
		If (ew_NotEmpty(sExtWrk)) Then sFilterList &= "&nbsp;&nbsp;" & sExtWrk & "<br>"
		If (ew_NotEmpty(sWrk)) Then sFilterList &= "&nbsp;&nbsp;" & sWrk & "<br>"

		' Field Periodo
		sExtWrk = ""
		sWrk = ""
		BuildDropDownFilter(Postergados.Periodo, sExtWrk, "")
		If (ew_NotEmpty(sExtWrk) OrElse ew_NotEmpty(sWrk)) Then sFilterList &= Postergados.Periodo.FldCaption() & "<br>"
		If (ew_NotEmpty(sExtWrk)) Then sFilterList &= "&nbsp;&nbsp;" & sExtWrk & "<br>"
		If (ew_NotEmpty(sWrk)) Then sFilterList &= "&nbsp;&nbsp;" & sWrk & "<br>"

		' Field Codigo
		sExtWrk = ""
		sWrk = ""
		BuildDropDownFilter(Postergados.Codigo, sExtWrk, "")
		If (ew_NotEmpty(sExtWrk) OrElse ew_NotEmpty(sWrk)) Then sFilterList &= Postergados.Codigo.FldCaption() & "<br>"
		If (ew_NotEmpty(sExtWrk)) Then sFilterList &= "&nbsp;&nbsp;" & sExtWrk & "<br>"
		If (ew_NotEmpty(sWrk)) Then sFilterList &= "&nbsp;&nbsp;" & sWrk & "<br>"

		' Field Estado Academico
		sExtWrk = ""
		sWrk = ""
		BuildDropDownFilter(Postergados.Estado_Academico, sExtWrk, "")
		If (ew_NotEmpty(sExtWrk) OrElse ew_NotEmpty(sWrk)) Then sFilterList &= Postergados.Estado_Academico.FldCaption() & "<br>"
		If (ew_NotEmpty(sExtWrk)) Then sFilterList &= "&nbsp;&nbsp;" & sExtWrk & "<br>"
		If (ew_NotEmpty(sWrk)) Then sFilterList &= "&nbsp;&nbsp;" & sWrk & "<br>"

		' Field Estado Final
		sExtWrk = ""
		sWrk = ""
		BuildDropDownFilter(Postergados.Estado_Final, sExtWrk, "")
		If (ew_NotEmpty(sExtWrk) OrElse ew_NotEmpty(sWrk)) Then sFilterList &= Postergados.Estado_Final.FldCaption() & "<br>"
		If (ew_NotEmpty(sExtWrk)) Then sFilterList &= "&nbsp;&nbsp;" & sExtWrk & "<br>"
		If (ew_NotEmpty(sWrk)) Then sFilterList &= "&nbsp;&nbsp;" & sWrk & "<br>"

		' Show Filters
		If (ew_NotEmpty(sFilterList)) Then
			ew_Write("CURRENT FILTERS:<br>")
			ew_Write(sFilterList)
		End If
	End Sub

	' Return popup filter
	Public Function GetPopupFilter() As String
		Dim sWrk As String = ""
		Return sWrk
	End Function

	' Return Sort parameters based on Sort links clicked
	Public Function GetSort() As String
		Dim bCtrl As Boolean = False
		Dim sOrder As String, sOrderType As String, sOrderBy As String
		Dim sSortField As String, sLastSort As String, sThisSort As String = "", sSortSql As String = ""

		' Check for a resetsort command
		If ew_NotEmpty(ew_Get("cmd")) Then
			Dim sCmd As String = ew_Get("cmd")
			If ew_SameText(sCmd, "resetsort") Then
				Postergados.OrderBy = ""
				Postergados.StartGroup = 1
				Postergados.Rut.Sort = ""
				Postergados.Nombre.Sort = ""
				Postergados.Paterno.Sort = ""
				Postergados.Materno.Sort = ""
				Postergados.Codigo_Carrera.Sort = ""
				Postergados.AF1o.Sort = ""
				Postergados.Periodo.Sort = ""
				Postergados.Codigo.Sort = ""
				Postergados.Estado_Academico.Sort = ""
				Postergados.Estado_Final.Sort = ""
				Postergados.Descripcion.Sort = ""
				Postergados.Fecha_ResoluciF3n.Sort = ""
				Postergados.ObservaciF3n.Sort = ""
			End If

		' Check for an Order parameter
		ElseIf (ew_NotEmpty(ew_Get("order"))) Then
			Postergados.CurrentOrder = ew_Get("order")
			Postergados.CurrentOrderType = ew_Get("ordertype")
			sSortSql = Postergados.SortSql()
			Postergados.OrderBy = sSortSql
			Postergados.StartGroup = 1
		End If

		' Set up default sort
		If (ew_Empty(Postergados.OrderBy)) Then
			Postergados.OrderBy = "[Paterno] ASC, [Materno] ASC, [Rut] ASC"
			Postergados.Paterno.Sort = "ASC"
			Postergados.Materno.Sort = "ASC"
			Postergados.Rut.Sort = "ASC"
		End If
		Return Postergados.OrderBy
	End Function

		' Page Load event
		Public Sub Page_Load()

			'ew_Write("Page Load")
		End Sub

		' Page Unload event
		Public Sub Page_Unload()

			'ew_Write("Page Unload")
		End Sub

		' Message Showing event
		Public Sub Message_Showing(ByRef msg As String)

			'msg = newmsg
		End Sub

		' Page Data Rendering event
		Public Sub Page_DataRendering(ByRef header As String)

			' Example:
			'header = "your header"

		End Sub

		' Page Data Rendered event
		Public Sub Page_DataRendered(ByRef footer As String)

			' Example:
			'footer = "your footer"

		End Sub

		' Form Custom Validate event
		Public Function Form_CustomValidate(ByRef CustomError As String) As Boolean

			'Return error message in CustomError
			Return True
		End Function
	End Class

	'
	' ASP.NET Page_Load event
	'

	Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) 
		gsDebugMsg = ""

		' Page init
		Postergados_summary = New crPostergados_summary(Me)
		Postergados_summary.Page_Init()

		' Set buffer/cache option
		Response.Buffer = EWRPT_RESPONSE_BUFFER
		Response.Cache.SetCacheability(HttpCacheability.NoCache)

		' Page main processing
		Postergados_summary.Page_Main()
	End Sub

	'
	' ASP.NET Page_Unload event
	'

	Protected Sub Page_Unload(ByVal sender As Object, ByVal e As System.EventArgs) 

		' Dispose page object
		If (Postergados_summary IsNot Nothing) Then Postergados_summary.Dispose()
	End Sub
</script>
<asp:Content ID="Content" ContentPlaceHolderID="ReportContent" runat="server">
<% If (Postergados.Export = "") Then %>
<script type="text/javascript">
var EWRPT_DATE_SEPARATOR = "/";
if (EWRPT_DATE_SEPARATOR == "") EWRPT_DATE_SEPARATOR = "/"; // Default date separator
</script>
<script type="text/javascript" src="aspxrptjs/ewrpt.js"></script>
<script type="text/javascript">
// Create page object
var Postergados_summary = new ewrpt_Page("Postergados_summary");
// page properties
Postergados_summary.PageID = "summary"; // page ID
Postergados_summary.FormID = "fPostergadossummaryfilter"; // form ID
var EWRPT_PAGE_ID = Postergados_summary.PageID;
// extend page with ValidateForm function
Postergados_summary.ValidateForm = function(fobj) {
	if (!this.ValidateRequired)
		return true; // ignore validation
	// Call Form Custom Validate event
	if (!this.Form_CustomValidate(fobj)) return false;
	return true;
}
// extend page with Form_CustomValidate function
Postergados_summary.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!
 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
<% If (EWRPT_CLIENT_VALIDATE) Then %>
Postergados_summary.ValidateRequired = true; // uses JavaScript validation
<% Else %>
Postergados_summary.ValidateRequired = false; // no JavaScript validation
<% End If %>
</script>
<script language="JavaScript" type="text/javascript">
<!--
// Write your client script here, no need to add script tags.
// To include another .js script, use:
// ew_ClientScriptInclude("my_javascript.js"); 
//-->
</script>
<% End If %>
<% Postergados_summary.ShowPageHeader() %>
<script src="FusionChartsFree/JSClass/FusionCharts.js" type="text/javascript"></script>
<% If (Postergados.Export = "") Then %>
<script src="aspxrptjs/popup.js" type="text/javascript"></script>
<script src="aspxrptjs/ewrptpop.js" type="text/javascript"></script>
<script type="text/javascript">
// popup fields
</script>
<% End If %>
<% If (Postergados.Export = "") Then %>
<!-- Table Container (Begin) -->
<table id="ewContainer" cellspacing="0" cellpadding="0" border="0">
<!-- Top Container (Begin) -->
<tr><td colspan="3"><div id="ewTop" class="aspnetreportmaker">
<!-- top slot -->
<a name="top"></a>
<% End If %>
<div id="underline"><h1><%= Postergados.TableCaption() %></h1></div>
<% If (Postergados.Export = "") Then %>
&nbsp;&nbsp;<a href="<%= Postergados_summary.ExportExcelUrl %>"><img src="images/excel.png" border="0" alt="Exportar a Excel" title="Exportar a Excel" /></a>
<% If (Postergados_summary.FilterApplied) Then %>
&nbsp;&nbsp;
<% End If %>
<% End If %>
<br><br>
<% Postergados_summary.ShowMessage() %>
<% If (Postergados.Export = "") Then %>
</div></td></tr>
<!-- Top Container (End) -->
<tr>
	<!-- Left Container (Begin) -->
	<td style="vertical-align: top;"><div id="ewLeft" class="aspnetreportmaker">
	<!-- Left slot -->
<% End If %>
<% If (Postergados.Export = "") Then %>
	</div></td>
	<!-- Left Container (End) -->
	<!-- Center Container - Report (Begin) -->
	<td style="vertical-align: top;" class="ewPadding"><div id="ewCenter" class="aspnetreportmaker">
	<!-- center slot -->
<% End If %>
<!-- summary report starts -->
<div id="report_summary">
<p>
  <% If (Postergados.Export = "") Then %>
  <%
Dim sButtonImage As String, sDivDisplay As String
If (Postergados.FilterPanelOption = 2 OrElse (Postergados.FilterPanelOption = 3 AndAlso Postergados_summary.FilterApplied) OrElse Postergados_summary.Filter = "0=101") Then
	sButtonImage = "aspxrptimages/collapse.gif"
	sDivDisplay = ""
Else 
	sButtonImage = "aspxrptimages/expand.gif"
	sDivDisplay = " style=""display: none;"""
End If
%>
  <a href="javascript:ewrpt_ToggleFilterPanel();" style="text-decoration: none;"><img id="ewrptToggleFilterImg" src="<%= sButtonImage %>" alt="" width="9" height="9" border="0"></a><span class="aspnetreportmaker">&nbsp;<%= ReportLanguage.Phrase("Filters") %></span></p>
<p><a href="Postergadossmry.aspx?cmd=reset"><%= ReportLanguage.Phrase("ResetAllFilter") %></a><br><br>
</p>
<div id="ewrptExtFilterPanel"<%= sDivDisplay %>>
  <!-- Search form (begin) -->
  <form name="fPostergadossummaryfilter" id="fPostergadossummaryfilter" action="Postergadossmry.aspx" class="ewForm" onsubmit="return Postergados_summary.ValidateForm(this);">
<table id="ewRptExtFilterTable" class="ewRptExtFilter">
<%

' Popup Filter
Dim cntf As Integer = Postergados.Materno.CustomFilters.Count
Dim totcnt As Integer, wrkcnt As Integer
%>
	<tr>
		<td><span class="aspnetreportmaker"><%= Postergados.Rut.FldCaption() %></span></td>
		<td>&nbsp;</td>
		<td>
			<table cellspacing="0" class="ewItemTable"><tr>
				<td><span class="aspnetreportmaker">
<input type="text" name="sv1_Rut" id="sv1_Rut" size="30" maxlength="30" value="<%= ew_HtmlEncode(Postergados.Rut.SearchValue) %>"<%= IIf(Postergados_summary.ClearExtFilter = "Postergados_Rut", " class=""ewInputCleared""", "") %>>
</span></td>
				<td></td>
			</tr></table>			
		</td>
	</tr>
	<tr>
		<td><span class="aspnetreportmaker"><%= Postergados.Nombre.FldCaption() %></span></td>
		<td>&nbsp;</td>
		<td>
			<table cellspacing="0" class="ewItemTable"><tr>
				<td><span class="aspnetreportmaker">
<input type="text" name="sv1_Nombre" id="sv1_Nombre" size="30" maxlength="32" value="<%= ew_HtmlEncode(Postergados.Nombre.SearchValue) %>"<%= IIf(Postergados_summary.ClearExtFilter = "Postergados_Nombre", " class=""ewInputCleared""", "") %>>
</span></td>
				<td></td>
			</tr></table>			
		</td>
	</tr>
	<tr>
		<td><span class="aspnetreportmaker"><%= Postergados.Paterno.FldCaption() %></span></td>
		<td>&nbsp;</td>
		<td>
			<table cellspacing="0" class="ewItemTable"><tr>
				<td><span class="aspnetreportmaker">
<input type="text" name="sv1_Paterno" id="sv1_Paterno" size="30" maxlength="30" value="<%= ew_HtmlEncode(Postergados.Paterno.SearchValue) %>"<%= IIf(Postergados_summary.ClearExtFilter = "Postergados_Paterno", " class=""ewInputCleared""", "") %>>
</span></td>
				<td></td>
			</tr></table>			
		</td>
	</tr>
	<tr>
		<td><span class="aspnetreportmaker"><%= Postergados.Materno.FldCaption() %></span></td>
		<td>&nbsp;</td>
		<td>
			<table cellspacing="0" class="ewItemTable"><tr>
				<td><span class="aspnetreportmaker">
<input type="text" name="sv1_Materno" id="sv1_Materno" size="30" maxlength="30" value="<%= ew_HtmlEncode(Postergados.Materno.SearchValue) %>"<%= IIf(Postergados_summary.ClearExtFilter = "Postergados_Materno", " class=""ewInputCleared""", "") %>>
</span></td>
				<td></td>
			</tr></table>			
		</td>
	</tr>
	<tr>
		<td><span class="aspnetreportmaker"><%= Postergados.Codigo_Carrera.FldCaption() %></span></td>
		<td></td>
		<td colspan="4"><span class="ewRptSearchOpr">
		<select name="sv_Codigo_Carrera" id="sv_Codigo_Carrera"<%= IIf(Postergados_summary.ClearExtFilter = "Postergados_Codigo_Carrera", " class=""ewInputCleared""", "") %>>
		<option value="<%= EWRPT_ALL_VALUE %>"<% If (ewrpt_MatchedFilterValue(Postergados.Codigo_Carrera.DropDownValue, EWRPT_ALL_VALUE)) Then Response.Write(" selected=""selected""") %>><%= ReportLanguage.Phrase("PleaseSelect") %></option>
<%

' Extended Filters
totcnt = Postergados.Codigo_Carrera.CustomFilters.Count + Postergados.Codigo_Carrera.DropDownList.Count
wrkcnt = 0

' Custom filters
For Each CustomFilter As crCustomFilter In Postergados.Codigo_Carrera.CustomFilters
	If (ew_SameStr(CustomFilter.FldName, "Codigo Carrera")) Then		
%>
		<option value="<%= "@@" & CustomFilter.FilterName %>"<% If (ewrpt_MatchedFilterValue(Postergados.Codigo_Carrera.DropDownValue, "@@" & CustomFilter.FilterName)) Then Response.Write(" selected=""selected""") %>><%= CustomFilter.DisplayName %></option>
<%
		wrkcnt += 1
	End If
Next
For Each value As Object In Postergados.Codigo_Carrera.DropDownList		
%>
		<option value="<%= value %>"<% If (ewrpt_MatchedFilterValue(Postergados.Codigo_Carrera.DropDownValue, value)) Then Response.Write(" selected=""selected""") %>><%= ewrpt_DropDownDisplayValue(value, "", 0) %></option>
<%
		wrkcnt += 1
Next
%>
		</select>
		</span></td>
	</tr>
	<tr>
		<td><span class="aspnetreportmaker"><%= Postergados.AF1o.FldCaption() %></span></td>
		<td></td>
		<td colspan="4"><span class="ewRptSearchOpr">
		<select name="sv_AF1o" id="sv_AF1o"<%= IIf(Postergados_summary.ClearExtFilter = "Postergados_AF1o", " class=""ewInputCleared""", "") %>>
		<option value="<%= EWRPT_ALL_VALUE %>"<% If (ewrpt_MatchedFilterValue(Postergados.AF1o.DropDownValue, EWRPT_ALL_VALUE)) Then Response.Write(" selected=""selected""") %>><%= ReportLanguage.Phrase("PleaseSelect") %></option>
<%

' Extended Filters
totcnt = Postergados.AF1o.CustomFilters.Count + Postergados.AF1o.DropDownList.Count
wrkcnt = 0

' Custom filters
For Each CustomFilter As crCustomFilter In Postergados.AF1o.CustomFilters
	If (ew_SameStr(CustomFilter.FldName, "Año")) Then		
%>
		<option value="<%= "@@" & CustomFilter.FilterName %>"<% If (ewrpt_MatchedFilterValue(Postergados.AF1o.DropDownValue, "@@" & CustomFilter.FilterName)) Then Response.Write(" selected=""selected""") %>><%= CustomFilter.DisplayName %></option>
<%
		wrkcnt += 1
	End If
Next
For Each value As Object In Postergados.AF1o.DropDownList		
%>
		<option value="<%= value %>"<% If (ewrpt_MatchedFilterValue(Postergados.AF1o.DropDownValue, value)) Then Response.Write(" selected=""selected""") %>><%= ewrpt_DropDownDisplayValue(value, "", 0) %></option>
<%
		wrkcnt += 1
Next
%>
		</select>
		</span></td>
	</tr>
	<tr>
		<td><span class="aspnetreportmaker"><%= Postergados.Periodo.FldCaption() %></span></td>
		<td></td>
		<td colspan="4"><span class="ewRptSearchOpr">
		<select name="sv_Periodo" id="sv_Periodo"<%= IIf(Postergados_summary.ClearExtFilter = "Postergados_Periodo", " class=""ewInputCleared""", "") %>>
		<option value="<%= EWRPT_ALL_VALUE %>"<% If (ewrpt_MatchedFilterValue(Postergados.Periodo.DropDownValue, EWRPT_ALL_VALUE)) Then Response.Write(" selected=""selected""") %>><%= ReportLanguage.Phrase("PleaseSelect") %></option>
<%

' Extended Filters
totcnt = Postergados.Periodo.CustomFilters.Count + Postergados.Periodo.DropDownList.Count
wrkcnt = 0

' Custom filters
For Each CustomFilter As crCustomFilter In Postergados.Periodo.CustomFilters
	If (ew_SameStr(CustomFilter.FldName, "Periodo")) Then		
%>
		<option value="<%= "@@" & CustomFilter.FilterName %>"<% If (ewrpt_MatchedFilterValue(Postergados.Periodo.DropDownValue, "@@" & CustomFilter.FilterName)) Then Response.Write(" selected=""selected""") %>><%= CustomFilter.DisplayName %></option>
<%
		wrkcnt += 1
	End If
Next
For Each value As Object In Postergados.Periodo.DropDownList		
%>
		<option value="<%= value %>"<% If (ewrpt_MatchedFilterValue(Postergados.Periodo.DropDownValue, value)) Then Response.Write(" selected=""selected""") %>><%= ewrpt_DropDownDisplayValue(value, "", 0) %></option>
<%
		wrkcnt += 1
Next
%>
		</select>
		</span></td>
	</tr>
	<tr>
		<td><span class="aspnetreportmaker"><%= Postergados.Codigo.FldCaption() %></span></td>
		<td></td>
		<td colspan="4"><span class="ewRptSearchOpr">
		<select name="sv_Codigo" id="sv_Codigo"<%= IIf(Postergados_summary.ClearExtFilter = "Postergados_Codigo", " class=""ewInputCleared""", "") %>>
		<option value="<%= EWRPT_ALL_VALUE %>"<% If (ewrpt_MatchedFilterValue(Postergados.Codigo.DropDownValue, EWRPT_ALL_VALUE)) Then Response.Write(" selected=""selected""") %>><%= ReportLanguage.Phrase("PleaseSelect") %></option>
<%

' Extended Filters
totcnt = Postergados.Codigo.CustomFilters.Count + Postergados.Codigo.DropDownList.Count
wrkcnt = 0

' Custom filters
For Each CustomFilter As crCustomFilter In Postergados.Codigo.CustomFilters
	If (ew_SameStr(CustomFilter.FldName, "Codigo")) Then		
%>
		<option value="<%= "@@" & CustomFilter.FilterName %>"<% If (ewrpt_MatchedFilterValue(Postergados.Codigo.DropDownValue, "@@" & CustomFilter.FilterName)) Then Response.Write(" selected=""selected""") %>><%= CustomFilter.DisplayName %></option>
<%
		wrkcnt += 1
	End If
Next
For Each value As Object In Postergados.Codigo.DropDownList		
%>
		<option value="<%= value %>"<% If (ewrpt_MatchedFilterValue(Postergados.Codigo.DropDownValue, value)) Then Response.Write(" selected=""selected""") %>><%= ewrpt_DropDownDisplayValue(value, "", 0) %></option>
<%
		wrkcnt += 1
Next
%>
		</select>
		</span></td>
	</tr>
	<tr>
		<td><span class="aspnetreportmaker"><%= Postergados.Estado_Academico.FldCaption() %></span></td>
		<td></td>
		<td colspan="4"><span class="ewRptSearchOpr">
		<select name="sv_Estado_Academico" id="sv_Estado_Academico"<%= IIf(Postergados_summary.ClearExtFilter = "Postergados_Estado_Academico", " class=""ewInputCleared""", "") %>>
		<option value="<%= EWRPT_ALL_VALUE %>"<% If (ewrpt_MatchedFilterValue(Postergados.Estado_Academico.DropDownValue, EWRPT_ALL_VALUE)) Then Response.Write(" selected=""selected""") %>><%= ReportLanguage.Phrase("PleaseSelect") %></option>
<%

' Extended Filters
totcnt = Postergados.Estado_Academico.CustomFilters.Count + Postergados.Estado_Academico.DropDownList.Count
wrkcnt = 0

' Custom filters
For Each CustomFilter As crCustomFilter In Postergados.Estado_Academico.CustomFilters
	If (ew_SameStr(CustomFilter.FldName, "Estado Academico")) Then		
%>
		<option value="<%= "@@" & CustomFilter.FilterName %>"<% If (ewrpt_MatchedFilterValue(Postergados.Estado_Academico.DropDownValue, "@@" & CustomFilter.FilterName)) Then Response.Write(" selected=""selected""") %>><%= CustomFilter.DisplayName %></option>
<%
		wrkcnt += 1
	End If
Next
For Each value As Object In Postergados.Estado_Academico.DropDownList		
%>
		<option value="<%= value %>"<% If (ewrpt_MatchedFilterValue(Postergados.Estado_Academico.DropDownValue, value)) Then Response.Write(" selected=""selected""") %>><%= ewrpt_DropDownDisplayValue(value, "", 0) %></option>
<%
		wrkcnt += 1
Next
%>
		</select>
		</span></td>
	</tr>
	<tr>
		<td><span class="aspnetreportmaker"><%= Postergados.Estado_Final.FldCaption() %></span></td>
		<td></td>
		<td colspan="4"><span class="ewRptSearchOpr">
		<select name="sv_Estado_Final" id="sv_Estado_Final"<%= IIf(Postergados_summary.ClearExtFilter = "Postergados_Estado_Final", " class=""ewInputCleared""", "") %>>
		<option value="<%= EWRPT_ALL_VALUE %>"<% If (ewrpt_MatchedFilterValue(Postergados.Estado_Final.DropDownValue, EWRPT_ALL_VALUE)) Then Response.Write(" selected=""selected""") %>><%= ReportLanguage.Phrase("PleaseSelect") %></option>
<%

' Extended Filters
totcnt = Postergados.Estado_Final.CustomFilters.Count + Postergados.Estado_Final.DropDownList.Count
wrkcnt = 0

' Custom filters
For Each CustomFilter As crCustomFilter In Postergados.Estado_Final.CustomFilters
	If (ew_SameStr(CustomFilter.FldName, "Estado Final")) Then		
%>
		<option value="<%= "@@" & CustomFilter.FilterName %>"<% If (ewrpt_MatchedFilterValue(Postergados.Estado_Final.DropDownValue, "@@" & CustomFilter.FilterName)) Then Response.Write(" selected=""selected""") %>><%= CustomFilter.DisplayName %></option>
<%
		wrkcnt += 1
	End If
Next
For Each value As Object In Postergados.Estado_Final.DropDownList		
%>
		<option value="<%= value %>"<% If (ewrpt_MatchedFilterValue(Postergados.Estado_Final.DropDownValue, value)) Then Response.Write(" selected=""selected""") %>><%= ewrpt_DropDownDisplayValue(value, "", 0) %></option>
<%
		wrkcnt += 1
Next
%>
		</select>
		</span></td>
	</tr>
</table>
<table class="ewRptExtFilter">
	<tr>
		<td><span class="aspnetreportmaker">
			<input type="submit" name="Submit" id="Submit" value="<%= ReportLanguage.Phrase("Search") %>">			
		</span></td>
	</tr>
</table>
</form>
<!-- Search form (end) -->
</div>
<br>
<% End If %>
<% If (Postergados.ShowCurrentFilter) Then %>
<div id="ewrptFilterList">
<% Postergados_summary.ShowFilterList() %>
</div>
<br>
<% End If %>
<table class="ewGrid" cellspacing="0"><tr>
	<td class="ewGridContent">
<!-- Report Grid (Begin) -->
<div class="ewGridMiddlePanel">
<table class="ewTable ewTableSeparate" cellspacing="0">
<%

' Set the last group to display if not export all
If (Postergados.ExportAll AndAlso ew_NotEmpty(Postergados.Export)) Then
	Postergados_summary.StopGrp = Postergados_summary.TotalGrps
Else
	Postergados_summary.StopGrp = Postergados_summary.StartGrp + Postergados_summary.DisplayGrps - 1
End If

' Stop group <= total number of groups
If (Postergados_summary.StopGrp > Postergados_summary.TotalGrps) Then
	Postergados_summary.StopGrp = Postergados_summary.TotalGrps
End If
Postergados_summary.RecCount = 0

' Get first row
If (Postergados_summary.TotalGrps > 0) Then
	Postergados_summary.GetRow() ' ASPXRPT
	Postergados_summary.GrpCount = 1
End If
While ((Postergados_summary.HasRow AndAlso Postergados_summary.GrpIndex < Postergados_summary.StopGrp) OrElse Postergados_summary.ShowFirstHeader)

	' Show header
	If (Postergados_summary.ShowFirstHeader) Then
%>
	<thead>
	<tr>
<td class="ewTableHeader">
<% If (ew_NotEmpty(Postergados.Export)) Then %>
<%= Postergados.Rut.FldCaption() %>
<% Else %>
	<table cellspacing="0" class="ewTableHeaderBtn"><tr>
<% If (ew_Empty(Postergados.SortUrl(Postergados.Rut))) Then %>
		<td style="vertical-align: bottom;"><%= Postergados.Rut.FldCaption() %></td>
<% Else %>
		<td class="ewPointer" onmousedown="ewrpt_Sort(event,'<%= Postergados.SortUrl(Postergados.Rut) %>',0);"><%= Postergados.Rut.FldCaption() %></td><td style="width: 10px;">
		<% If (Postergados.Rut.Sort = "ASC") Then %><img src="aspxrptimages/sortup.gif" width="10" height="9" border="0"><% ElseIf (Postergados.Rut.Sort = "DESC") Then %><img src="aspxrptimages/sortdown.gif" width="10" height="9" border="0"><% End If %></td>
<% End If %>
	</tr></table>
<% End If %>
</td>
<td class="ewTableHeader">
<% If (ew_NotEmpty(Postergados.Export)) Then %>
<%= Postergados.Nombre.FldCaption() %>
<% Else %>
	<table cellspacing="0" class="ewTableHeaderBtn"><tr>
<% If (ew_Empty(Postergados.SortUrl(Postergados.Nombre))) Then %>
		<td style="vertical-align: bottom;"><%= Postergados.Nombre.FldCaption() %></td>
<% Else %>
		<td class="ewPointer" onmousedown="ewrpt_Sort(event,'<%= Postergados.SortUrl(Postergados.Nombre) %>',0);"><%= Postergados.Nombre.FldCaption() %></td><td style="width: 10px;">
		<% If (Postergados.Nombre.Sort = "ASC") Then %><img src="aspxrptimages/sortup.gif" width="10" height="9" border="0"><% ElseIf (Postergados.Nombre.Sort = "DESC") Then %><img src="aspxrptimages/sortdown.gif" width="10" height="9" border="0"><% End If %></td>
<% End If %>
	</tr></table>
<% End If %>
</td>
<td class="ewTableHeader">
<% If (ew_NotEmpty(Postergados.Export)) Then %>
<%= Postergados.Paterno.FldCaption() %>
<% Else %>
	<table cellspacing="0" class="ewTableHeaderBtn"><tr>
<% If (ew_Empty(Postergados.SortUrl(Postergados.Paterno))) Then %>
		<td style="vertical-align: bottom;"><%= Postergados.Paterno.FldCaption() %></td>
<% Else %>
		<td class="ewPointer" onmousedown="ewrpt_Sort(event,'<%= Postergados.SortUrl(Postergados.Paterno) %>',0);"><%= Postergados.Paterno.FldCaption() %></td><td style="width: 10px;">
		<% If (Postergados.Paterno.Sort = "ASC") Then %><img src="aspxrptimages/sortup.gif" width="10" height="9" border="0"><% ElseIf (Postergados.Paterno.Sort = "DESC") Then %><img src="aspxrptimages/sortdown.gif" width="10" height="9" border="0"><% End If %></td>
<% End If %>
	</tr></table>
<% End If %>
</td>
<td class="ewTableHeader">
<% If (ew_NotEmpty(Postergados.Export)) Then %>
<%= Postergados.Materno.FldCaption() %>
<% Else %>
	<table cellspacing="0" class="ewTableHeaderBtn"><tr>
<% If (ew_Empty(Postergados.SortUrl(Postergados.Materno))) Then %>
		<td style="vertical-align: bottom;"><%= Postergados.Materno.FldCaption() %></td>
<% Else %>
		<td class="ewPointer" onmousedown="ewrpt_Sort(event,'<%= Postergados.SortUrl(Postergados.Materno) %>',0);"><%= Postergados.Materno.FldCaption() %></td><td style="width: 10px;">
		<% If (Postergados.Materno.Sort = "ASC") Then %><img src="aspxrptimages/sortup.gif" width="10" height="9" border="0"><% ElseIf (Postergados.Materno.Sort = "DESC") Then %><img src="aspxrptimages/sortdown.gif" width="10" height="9" border="0"><% End If %></td>
<% End If %>
	</tr></table>
<% End If %>
</td>
<td class="ewTableHeader">
<% If (ew_NotEmpty(Postergados.Export)) Then %>
<%= Postergados.Codigo_Carrera.FldCaption() %>
<% Else %>
	<table cellspacing="0" class="ewTableHeaderBtn"><tr>
<% If (ew_Empty(Postergados.SortUrl(Postergados.Codigo_Carrera))) Then %>
		<td style="vertical-align: bottom;"><%= Postergados.Codigo_Carrera.FldCaption() %></td>
<% Else %>
		<td class="ewPointer" onmousedown="ewrpt_Sort(event,'<%= Postergados.SortUrl(Postergados.Codigo_Carrera) %>',0);"><%= Postergados.Codigo_Carrera.FldCaption() %></td><td style="width: 10px;">
		<% If (Postergados.Codigo_Carrera.Sort = "ASC") Then %><img src="aspxrptimages/sortup.gif" width="10" height="9" border="0"><% ElseIf (Postergados.Codigo_Carrera.Sort = "DESC") Then %><img src="aspxrptimages/sortdown.gif" width="10" height="9" border="0"><% End If %></td>
<% End If %>
	</tr></table>
<% End If %>
</td>
<td class="ewTableHeader">
<% If (ew_NotEmpty(Postergados.Export)) Then %>
<%= Postergados.AF1o.FldCaption() %>
<% Else %>
	<table cellspacing="0" class="ewTableHeaderBtn"><tr>
<% If (ew_Empty(Postergados.SortUrl(Postergados.AF1o))) Then %>
		<td style="vertical-align: bottom;"><%= Postergados.AF1o.FldCaption() %></td>
<% Else %>
		<td class="ewPointer" onmousedown="ewrpt_Sort(event,'<%= Postergados.SortUrl(Postergados.AF1o) %>',0);"><%= Postergados.AF1o.FldCaption() %></td><td style="width: 10px;">
		<% If (Postergados.AF1o.Sort = "ASC") Then %><img src="aspxrptimages/sortup.gif" width="10" height="9" border="0"><% ElseIf (Postergados.AF1o.Sort = "DESC") Then %><img src="aspxrptimages/sortdown.gif" width="10" height="9" border="0"><% End If %></td>
<% End If %>
	</tr></table>
<% End If %>
</td>
<td class="ewTableHeader">
<% If (ew_NotEmpty(Postergados.Export)) Then %>
<%= Postergados.Periodo.FldCaption() %>
<% Else %>
	<table cellspacing="0" class="ewTableHeaderBtn"><tr>
<% If (ew_Empty(Postergados.SortUrl(Postergados.Periodo))) Then %>
		<td style="vertical-align: bottom;"><%= Postergados.Periodo.FldCaption() %></td>
<% Else %>
		<td class="ewPointer" onmousedown="ewrpt_Sort(event,'<%= Postergados.SortUrl(Postergados.Periodo) %>',0);"><%= Postergados.Periodo.FldCaption() %></td><td style="width: 10px;">
		<% If (Postergados.Periodo.Sort = "ASC") Then %><img src="aspxrptimages/sortup.gif" width="10" height="9" border="0"><% ElseIf (Postergados.Periodo.Sort = "DESC") Then %><img src="aspxrptimages/sortdown.gif" width="10" height="9" border="0"><% End If %></td>
<% End If %>
	</tr></table>
<% End If %>
</td>
<td class="ewTableHeader">
<% If (ew_NotEmpty(Postergados.Export)) Then %>
<%= Postergados.Codigo.FldCaption() %>
<% Else %>
	<table cellspacing="0" class="ewTableHeaderBtn"><tr>
<% If (ew_Empty(Postergados.SortUrl(Postergados.Codigo))) Then %>
		<td style="vertical-align: bottom;"><%= Postergados.Codigo.FldCaption() %></td>
<% Else %>
		<td class="ewPointer" onmousedown="ewrpt_Sort(event,'<%= Postergados.SortUrl(Postergados.Codigo) %>',0);"><%= Postergados.Codigo.FldCaption() %></td><td style="width: 10px;">
		<% If (Postergados.Codigo.Sort = "ASC") Then %><img src="aspxrptimages/sortup.gif" width="10" height="9" border="0"><% ElseIf (Postergados.Codigo.Sort = "DESC") Then %><img src="aspxrptimages/sortdown.gif" width="10" height="9" border="0"><% End If %></td>
<% End If %>
	</tr></table>
<% End If %>
</td>
<td class="ewTableHeader">
<% If (ew_NotEmpty(Postergados.Export)) Then %>
<%= Postergados.Estado_Academico.FldCaption() %>
<% Else %>
	<table cellspacing="0" class="ewTableHeaderBtn"><tr>
<% If (ew_Empty(Postergados.SortUrl(Postergados.Estado_Academico))) Then %>
		<td style="vertical-align: bottom;"><%= Postergados.Estado_Academico.FldCaption() %></td>
<% Else %>
		<td class="ewPointer" onmousedown="ewrpt_Sort(event,'<%= Postergados.SortUrl(Postergados.Estado_Academico) %>',0);"><%= Postergados.Estado_Academico.FldCaption() %></td><td style="width: 10px;">
		<% If (Postergados.Estado_Academico.Sort = "ASC") Then %><img src="aspxrptimages/sortup.gif" width="10" height="9" border="0"><% ElseIf (Postergados.Estado_Academico.Sort = "DESC") Then %><img src="aspxrptimages/sortdown.gif" width="10" height="9" border="0"><% End If %></td>
<% End If %>
	</tr></table>
<% End If %>
</td>
<td class="ewTableHeader">
<% If (ew_NotEmpty(Postergados.Export)) Then %>
<%= Postergados.Estado_Final.FldCaption() %>
<% Else %>
	<table cellspacing="0" class="ewTableHeaderBtn"><tr>
<% If (ew_Empty(Postergados.SortUrl(Postergados.Estado_Final))) Then %>
		<td style="vertical-align: bottom;"><%= Postergados.Estado_Final.FldCaption() %></td>
<% Else %>
		<td class="ewPointer" onmousedown="ewrpt_Sort(event,'<%= Postergados.SortUrl(Postergados.Estado_Final) %>',0);"><%= Postergados.Estado_Final.FldCaption() %></td><td style="width: 10px;">
		<% If (Postergados.Estado_Final.Sort = "ASC") Then %><img src="aspxrptimages/sortup.gif" width="10" height="9" border="0"><% ElseIf (Postergados.Estado_Final.Sort = "DESC") Then %><img src="aspxrptimages/sortdown.gif" width="10" height="9" border="0"><% End If %></td>
<% End If %>
	</tr></table>
<% End If %>
</td>
<td class="ewTableHeader">
<% If (ew_NotEmpty(Postergados.Export)) Then %>
<%= Postergados.Descripcion.FldCaption() %>
<% Else %>
	<table cellspacing="0" class="ewTableHeaderBtn"><tr>
<% If (ew_Empty(Postergados.SortUrl(Postergados.Descripcion))) Then %>
		<td style="vertical-align: bottom;"><%= Postergados.Descripcion.FldCaption() %></td>
<% Else %>
		<td class="ewPointer" onmousedown="ewrpt_Sort(event,'<%= Postergados.SortUrl(Postergados.Descripcion) %>',0);"><%= Postergados.Descripcion.FldCaption() %></td><td style="width: 10px;">
		<% If (Postergados.Descripcion.Sort = "ASC") Then %><img src="aspxrptimages/sortup.gif" width="10" height="9" border="0"><% ElseIf (Postergados.Descripcion.Sort = "DESC") Then %><img src="aspxrptimages/sortdown.gif" width="10" height="9" border="0"><% End If %></td>
<% End If %>
	</tr></table>
<% End If %>
</td>
<td class="ewTableHeader">
<% If (ew_NotEmpty(Postergados.Export)) Then %>
<%= Postergados.Fecha_ResoluciF3n.FldCaption() %>
<% Else %>
	<table cellspacing="0" class="ewTableHeaderBtn"><tr>
<% If (ew_Empty(Postergados.SortUrl(Postergados.Fecha_ResoluciF3n))) Then %>
		<td style="vertical-align: bottom;"><%= Postergados.Fecha_ResoluciF3n.FldCaption() %></td>
<% Else %>
		<td class="ewPointer" onmousedown="ewrpt_Sort(event,'<%= Postergados.SortUrl(Postergados.Fecha_ResoluciF3n) %>',0);"><%= Postergados.Fecha_ResoluciF3n.FldCaption() %></td><td style="width: 10px;">
		<% If (Postergados.Fecha_ResoluciF3n.Sort = "ASC") Then %><img src="aspxrptimages/sortup.gif" width="10" height="9" border="0"><% ElseIf (Postergados.Fecha_ResoluciF3n.Sort = "DESC") Then %><img src="aspxrptimages/sortdown.gif" width="10" height="9" border="0"><% End If %></td>
<% End If %>
	</tr></table>
<% End If %>
</td>
<td class="ewTableHeader">
<% If (ew_NotEmpty(Postergados.Export)) Then %>
<%= Postergados.ObservaciF3n.FldCaption() %>
<% Else %>
	<table cellspacing="0" class="ewTableHeaderBtn"><tr>
<% If (ew_Empty(Postergados.SortUrl(Postergados.ObservaciF3n))) Then %>
		<td style="vertical-align: bottom;"><%= Postergados.ObservaciF3n.FldCaption() %></td>
<% Else %>
		<td class="ewPointer" onmousedown="ewrpt_Sort(event,'<%= Postergados.SortUrl(Postergados.ObservaciF3n) %>',0);"><%= Postergados.ObservaciF3n.FldCaption() %></td><td style="width: 10px;">
		<% If (Postergados.ObservaciF3n.Sort = "ASC") Then %><img src="aspxrptimages/sortup.gif" width="10" height="9" border="0"><% ElseIf (Postergados.ObservaciF3n.Sort = "DESC") Then %><img src="aspxrptimages/sortdown.gif" width="10" height="9" border="0"><% End If %></td>
<% End If %>
	</tr></table>
<% End If %>
</td>
	</tr>
	</thead>
	<tbody>
<%
		Postergados_summary.ShowFirstHeader = False
	End If
	Postergados_summary.RecCount += 1

		' Render detail row
		Postergados.ResetCSS()
		Postergados.RowType = EWRPT_ROWTYPE_DETAIL
		Postergados_summary.RenderRow()
%>
	<tr<%= Postergados.RowAttributes() %>>
		<td<%= Postergados.Rut.CellAttributes %>>
<div<%= Postergados.Rut.ViewAttributes%>><%= Postergados.Rut.ListViewValue%></div>
</td>
		<td<%= Postergados.Nombre.CellAttributes %>>
<div<%= Postergados.Nombre.ViewAttributes%>><%= Postergados.Nombre.ListViewValue%></div>
</td>
		<td<%= Postergados.Paterno.CellAttributes %>>
<div<%= Postergados.Paterno.ViewAttributes%>><%= Postergados.Paterno.ListViewValue%></div>
</td>
		<td<%= Postergados.Materno.CellAttributes %>>
<div<%= Postergados.Materno.ViewAttributes%>><%= Postergados.Materno.ListViewValue%></div>
</td>
		<td<%= Postergados.Codigo_Carrera.CellAttributes %>>
<div<%= Postergados.Codigo_Carrera.ViewAttributes%>><%= Postergados.Codigo_Carrera.ListViewValue%></div>
</td>
		<td<%= Postergados.AF1o.CellAttributes %>>
<div<%= Postergados.AF1o.ViewAttributes%>><%= Postergados.AF1o.ListViewValue%></div>
</td>
		<td<%= Postergados.Periodo.CellAttributes %>>
<div<%= Postergados.Periodo.ViewAttributes%>><%= Postergados.Periodo.ListViewValue%></div>
</td>
		<td<%= Postergados.Codigo.CellAttributes %>>
<div<%= Postergados.Codigo.ViewAttributes%>><%= Postergados.Codigo.ListViewValue%></div>
</td>
		<td<%= Postergados.Estado_Academico.CellAttributes %>>
<div<%= Postergados.Estado_Academico.ViewAttributes%>><%= Postergados.Estado_Academico.ListViewValue%></div>
</td>
		<td<%= Postergados.Estado_Final.CellAttributes %>>
<div<%= Postergados.Estado_Final.ViewAttributes%>><%= Postergados.Estado_Final.ListViewValue%></div>
</td>
		<td<%= Postergados.Descripcion.CellAttributes %>>
<div<%= Postergados.Descripcion.ViewAttributes%>><%= Postergados.Descripcion.ListViewValue%></div>
</td>
		<td<%= Postergados.Fecha_ResoluciF3n.CellAttributes %>>
<div<%= Postergados.Fecha_ResoluciF3n.ViewAttributes%>><%= Postergados.Fecha_ResoluciF3n.ListViewValue%></div>
</td>
		<td<%= Postergados.ObservaciF3n.CellAttributes %>>
<div<%= Postergados.ObservaciF3n.ViewAttributes%>><%= Postergados.ObservaciF3n.ListViewValue%></div>
</td>
	</tr>
<%

		' Accumulate page summary
		Postergados_summary.AccumulateSummary()

		' Get next record
		Postergados_summary.GetRow() ' ASPXRPT
		Postergados_summary.GrpCount += 1
End while
%>
	</tbody>
	<tfoot>
<%
If (Postergados_summary.TotalGrps > 0) Then
	Postergados.ResetCSS()
	Postergados.RowType = EWRPT_ROWTYPE_TOTAL
	Postergados.RowTotalType = EWRPT_ROWTOTAL_GRAND
	Postergados.RowTotalSubType = EWRPT_ROWTOTAL_FOOTER
	Postergados.RowAttrs("class") = "ewRptGrandSummary"
	Postergados_summary.RenderRow()
%>
	<!-- tr><td colspan="13"><span class="aspnetreportmaker">&nbsp;<br></span></td></tr -->
	<tr<%= Postergados.RowAttributes() %>><td colspan="13"><%= ReportLanguage.Phrase("RptGrandTotal") %> (<%= ewrpt_FormatNumber(Postergados_summary.TotCount,0) %> <%= ReportLanguage.Phrase("RptDtlRec") %>)</td></tr>
<% End If %>
	</tfoot>
</table>
</div>
<% If (Postergados.Export = "") Then %>
<div class="ewGridLowerPanel">
<form name="ewpagerform" id="ewpagerform" class="ewForm">
<table id="ewRptPagerTable" border="0" cellspacing="0" cellpadding="0">
	<tr>
		<td style="white-space: nowrap;">
<% If Postergados_summary.Pager Is Nothing Then Postergados_summary.Pager = New cPrevNextPager(Postergados_summary.StartGrp, Postergados_summary.DisplayGrps, Postergados_summary.TotalGrps) %>
<% If Postergados_summary.Pager.RecordCount > 0 Then %>
	<table border="0" cellspacing="0" cellpadding="0"><tr><td><span class="aspnetreportmaker"><%= ReportLanguage.Phrase("Page") %>&nbsp;</span></td>
<!--first page button-->
	<% If Postergados_summary.Pager.FirstButton.Enabled Then %>
	<td><a href="Postergadossmry.aspx?start=<%= Postergados_summary.Pager.FirstButton.Start %>"><img src="aspxrptimages/first.gif" alt="<%= ReportLanguage.Phrase("PagerFirst") %>" width="16" height="16" border="0"></a></td>
	<% Else %>
	<td><img src="aspxrptimages/firstdisab.gif" alt="<%= ReportLanguage.Phrase("PagerFirst") %>" width="16" height="16" border="0"></td>
	<% End If %>
<!--previous page button-->
	<% If Postergados_summary.Pager.PrevButton.Enabled Then %>
	<td><a href="Postergadossmry.aspx?start=<%= Postergados_summary.Pager.PrevButton.Start %>"><img src="aspxrptimages/prev.gif" alt="<%= ReportLanguage.Phrase("PagerPrevious") %>" width="16" height="16" border="0"></a></td>
	<% Else %>
	<td><img src="aspxrptimages/prevdisab.gif" alt="<%= ReportLanguage.Phrase("PagerPrevious") %>" width="16" height="16" border="0"></td>
	<% End If %>
<!--current page number-->
	<td><input type="text" name="pageno" id="pageno" value="<%= Postergados_summary.Pager.CurrentPage %>" size="4" /></td>
<!--next page button-->
	<% If Postergados_summary.Pager.NextButton.Enabled Then %>
	<td><a href="Postergadossmry.aspx?start=<%= Postergados_summary.Pager.NextButton.Start %>"><img src="aspxrptimages/next.gif" alt="<%= ReportLanguage.Phrase("PagerNext") %>" width="16" height="16" border="0"></a></td>
	<% Else %>
	<td><img src="aspxrptimages/nextdisab.gif" alt="<%= ReportLanguage.Phrase("PagerNext") %>" width="16" height="16" border="0"></td>
	<% End If %>
<!--last page button-->
	<% If Postergados_summary.Pager.LastButton.Enabled Then %>
	<td><a href="Postergadossmry.aspx?start=<%= Postergados_summary.Pager.LastButton.Start %>"><img src="aspxrptimages/last.gif" alt="<%= ReportLanguage.Phrase("PagerLast") %>" width="16" height="16" border="0"></a></td>	
	<% Else %>
	<td><img src="aspxrptimages/lastdisab.gif" alt="<%= ReportLanguage.Phrase("PagerLast") %>" width="16" height="16" border="0"></td>
	<% End If %>
	<td><span class="aspnetreportmaker">&nbsp;<%= ReportLanguage.Phrase("of") %> <%= Postergados_summary.Pager.PageCount %></span></td>
	</tr></table>
	</td>	
	<td>&nbsp;&nbsp;&nbsp;&nbsp;</td>
	<td>
	<span class="aspnetreportmaker"><%= ReportLanguage.Phrase("Record") %> <%= Postergados_summary.Pager.FromIndex %> <%= ReportLanguage.Phrase("To") %> <%= Postergados_summary.Pager.ToIndex %> <%= ReportLanguage.Phrase("Of") %> <%= Postergados_summary.Pager.RecordCount %></span>	
<% Else %>
	<% If Postergados_summary.Filter = "0=101" Then %>
	<span class="aspnetreportmaker"><%= ReportLanguage.Phrase("EnterSearchCriteria") %></span>
	<% Else %>
	<span class="aspnetreportmaker"><%= ReportLanguage.Phrase("NoRecord") %></span>
	<% End If %>
<% End If %>
		</td>
<% If (Postergados_summary.TotalGrps > 0) Then %>
		<td style="white-space: nowrap;">&nbsp;&nbsp;&nbsp;&nbsp;</td>
		<td align="right" style="vertical-align: top; white-space: nowrap;"><span class="aspnetreportmaker"><%= ReportLanguage.Phrase("GroupsPerPage") %>&nbsp;
<select id="<%= EWRPT_TABLE_GROUP_PER_PAGE %>" name="<%= EWRPT_TABLE_GROUP_PER_PAGE %>" class="aspnetreportmaker" onchange="this.form.submit();">
<option value="20"<% If Postergados_summary.DisplayGrps = 20 Then Response.Write(" selected=""selected""") %>>20</option>
<option value="50"<% If Postergados_summary.DisplayGrps = 50 Then Response.Write(" selected=""selected""") %>>50</option>
<option value="100"<% If Postergados_summary.DisplayGrps = 100 Then Response.Write(" selected=""selected""") %>>100</option>
</select>
		</span></td>
<% End If %>
	</tr>
</table>
</form>
</div>
<% End If %>
</td></tr></table>
</div>
<!-- Summary Report Ends -->
<% If (Postergados.Export = "") Then %>
	</div><br></td>
	<!-- Center Container - Report (End) -->
	<!-- Right Container (Begin) -->
	<td style="vertical-align: top;"><div id="ewRight" class="aspnetreportmaker">
	<!-- Right slot -->
<% End If %>
<% If (Postergados.Export = "") Then %>
	</div></td>
	<!-- Right Container (End) -->
</tr>
<!-- Bottom Container (Begin) -->
<tr><td colspan="3"><div id="ewBottom" class="aspnetreportmaker">
	<!-- Bottom slot -->
<% End If %>
<% If (Postergados.Export = "") Then %>
	</div><br></td></tr>
<!-- Bottom Container (End) -->
</table>
<!-- Table Container (End) -->
<% End If %>
<% Postergados_summary.ShowPageFooter() %>
<% If (EWRPT_DEBUG_ENABLED) Then ew_Write(ew_DebugMsg()) %>
<% If (Postergados.Export = "") Then %>
<script language="JavaScript" type="text/javascript">
<!--
// Write your table-specific startup script here
// document.write("page loaded");
//-->
</script>
<% End If %>
</asp:Content>
