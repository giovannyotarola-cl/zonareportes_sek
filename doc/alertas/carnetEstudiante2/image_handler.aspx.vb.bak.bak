...
Public Class image_handler
    ...
    Protected Sub CreateImage(ByVal id_alumno As String)
        ...
        strSQL = "SELECT FOTO from Matricula.foto_alumno WHERE CODCLI = '" + id_alumno + "';"
        System.Diagnostics.Debug.WriteLine(strSQL)

        conn = New SqlConnection()
        conn.ConnectionString = System.Configuration.ConfigurationManager.ConnectionStrings("MatriculaConnectionString").ConnectionString
        conn.Open()
        cmd = New SqlCommand(strSQL, conn)
        _buf = cmd.ExecuteScalar


        'NO CONTROLA BIEN LOS NULOS
