...
Public Class WebForm2
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        origen_sql_jerarquias.SelectCommand = "SELECT distinct dbo.SEK_EvDes_Users.username, dbo.SEK_EvDes_Users.nombre, dbo.SEK_EvDes_Jerarquia.id FROM dbo.SEK_EvDes_Users INNER JOIN dbo.SEK_EvDes_Jerarquia ON dbo.SEK_EvDes_Users.id = dbo.SEK_EvDes_Jerarquia.id_empleado WHERE (dbo.SEK_EvDes_Jerarquia.id_superior ='" + id_empleado.ToString + "')"
        'System.Diagnostics.Debug.WriteLine(origen_sql_jerarquias.SelectCommand)
        origen_sql_jerarquias.DeleteCommand = "Delete from dbo.SEK_EvDes_Jerarquia where id = @id"

        'Obtener lista de usuarios jerarquizables
        fill_dropdown.SelectCommand = "SELECT username, nombre as nombre_empleado, cargo, nivel, id FROM dbo.SEK_EvDes_Users WHERE (id <> '" + id_empleado.ToString + "') AND (nivel > " + nivel_empleado.ToString + " AND id not in (SELECT dbo.SEK_EvDes_Users.id FROM dbo.SEK_EvDes_Users INNER JOIN dbo.SEK_EvDes_Jerarquia ON dbo.SEK_EvDes_Users.id = dbo.SEK_EvDes_Jerarquia.id_empleado WHERE (dbo.SEK_EvDes_Jerarquia.id_superior = " + id_empleado.ToString + ") ))"
    ...
    Protected Sub getEmpleado()
        ...
        Try
            strSQL = "select id, nivel from dbo.SEK_EvDes_Users where id = " + Request.QueryString("empleado").ToString + ";"
    ...
    Protected Sub Button1_Click(ByVal sender As Object, ByVal e As EventArgs) Handles Button1.Click
        ...
        Try
            ...
            SQLInsert = "Insert into dbo.SEK_EvDes_Jerarquia values(" + id_empleado.ToString + ", " + lista_empleados.SelectedValue.ToString + ")"
