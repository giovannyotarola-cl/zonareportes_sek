...
Partial Class mostrarEvEstudianteDocente
    ...
    Protected Sub cargarEncuestaProfe()
        ...
        'La 8 y 9 se repiten , por lo cual solo se tomara la prenguta 8 y se imitira el 9'
        '" AS P9,AVG(CAST(P10 AS DECIMAL))" +'
        ...
        Dim strSql As String = "SELECT AVG(CAST(P1 AS DECIMAL)) AS P1 " +
            ...
            ", AVG((CAST(P1 AS DECIMAL) + CAST(P2 AS DECIMAL))/2) AS PromPlaneacion" +
            ", AVG((CAST(P3 AS DECIMAL) + CAST(P4 AS DECIMAL) + CAST(P5 AS DECIMAL))/3) AS PromMetodologia " +
            ", AVG((CAST(P6 AS DECIMAL) + CAST(P7 AS DECIMAL) + CAST(P8 AS DECIMAL))/3) AS PromContenidos" +
            ", AVG((CAST(P10 AS DECIMAL) + CAST(P11 AS DECIMAL) + CAST(P12 AS DECIMAL) + CAST(P13 AS DECIMAL))/4) AS PromEvaluacion" +
            ", AVG((CAST(P14 AS DECIMAL) + CAST(P15 AS DECIMAL) + CAST(P16 AS DECIMAL) + CAST(P17 AS DECIMAL))/4) AS PromDeberes " +
            " FROM dbo.SEK_EPdocentePE WHERE  CODPROF='" + profesor + "' " +
            "AND ANO='" + ano + "' AND PERIODO='" + periodo + "' AND ASIGNATURA='" + asignatura + "' AND SECCION='" + seccion + "'"
        Try
            conn.Open()
            ...
            lblEval1.Text = Mid(ds.Tables(0).Rows(0)("p10").ToString(), 1, decimales)
            lblEval2.Text = Mid(ds.Tables(0).Rows(0)("p11").ToString(), 1, decimales)
            lblEval3.Text = Mid(ds.Tables(0).Rows(0)("p12").ToString(), 1, decimales)
            lblEval4.Text = Mid(ds.Tables(0).Rows(0)("p13").ToString(), 1, decimales)

            lblDeberes1.Text = Mid(ds.Tables(0).Rows(0)("p14").ToString(), 1, decimales)
            lblDeberes2.Text = Mid(ds.Tables(0).Rows(0)("p15").ToString(), 1, decimales)
            lblDeberes3.Text = Mid(ds.Tables(0).Rows(0)("p16").ToString(), 1, decimales)
            lblDeberes4.Text = Mid(ds.Tables(0).Rows(0)("p17").ToString(), 1, decimales)
        ...
        strSql = "SELECT" +
            ...
            ",AVG((CAST(P10 AS DECIMAL) + CAST(P11 AS DECIMAL) + CAST(P12 AS DECIMAL) + CAST(P13 AS DECIMAL))/4) AS PromEvaluacion" +
            ",AVG((CAST(P14 AS DECIMAL) + CAST(P15 AS DECIMAL) + CAST(P16 AS DECIMAL) + CAST(P17 AS DECIMAL))/4) AS PromDeberes " +
            "FROM dbo.SEK_EPdocentePE WHERE CARRERA='" + carrera + "' AND ANO='" + ano + "' AND PERIODO='" + periodo + "'"
        Try
            conn.Open()
            da = New SqlDataAdapter(strSql, conn)
            da.Fill(ds2)
            ...
            lblEval1Carrera.Text = Mid(ds2.Tables(0).Rows(0)("P10").ToString(), 1, decimales)
            lblEval2Carrera.Text = Mid(ds2.Tables(0).Rows(0)("P11").ToString(), 1, decimales)
            lblEval3Carrera.Text = Mid(ds2.Tables(0).Rows(0)("P12").ToString(), 1, decimales)
            lblEval4Carrera.Text = Mid(ds2.Tables(0).Rows(0)("P13").ToString(), 1, decimales)

            lblDeberes1Carrera.Text = Mid(ds2.Tables(0).Rows(0)("P14").ToString(), 1, decimales)
            lblDeberes2Carrera.Text = Mid(ds2.Tables(0).Rows(0)("P15").ToString(), 1, decimales)
            lblDeberes3Carrera.Text = Mid(ds2.Tables(0).Rows(0)("P16").ToString(), 1, decimales)
            lblDeberes4Carrera.Text = Mid(ds2.Tables(0).Rows(0)("P17").ToString(), 1, decimales)
    ...
    Protected Function encuestasHechas(ByVal ano As String, ByVal periodo As String, ByVal codProf As String, ByVal codRamo As String, ByVal seccion As String) As String
        ...
        strSql = "Select count(*) as Total from dbo.SEK_EAdocentePE where ano='" + ano + "' and periodo='" + periodo + "' and codRamo='" + codRamo + "' and CODPROF='" + codProf + "' and seccion='" + seccion + "'"
        Try
            conn.Open()
            da = New SqlDataAdapter(strSql, conn)
            da.Fill(ds)
