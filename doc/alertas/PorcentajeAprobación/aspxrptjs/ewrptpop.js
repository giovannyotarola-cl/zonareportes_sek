// Popup panel for ASP.NET Report Maker 4
...
function ewrpt_SetPopupContent(name, data, useRange, rangeFrom, rangeTo) {
	...
	if (useRange) {
		var selected;
		html += "<table border=\"0\" cellspacing=\"1\" cellpadding=\"1\" class=\"aspnetreportmaker\">";
	...
	html += "<table width=\"100%\" border=\"0\" cellspacing=\"1\" cellpadding=\"1\" class=\"aspnetreportmaker\"><tr><td>";
	html += "<input type=\"checkbox\" name=\"sel_" + name + "\" value=\"\" onclick=\"ewrpt_SelectAll(this);\"" + checkedall + " />" + ewLanguage.Phrase("PopupAll") + "<br>"; // ASPXRPT
	sb.clear();	
	for (var i=0; i<datacnt; i++) {
		...
		sb.append("<input type=\"checkbox\" name=\"sel_" + name + "\" value=\"" + data[i][0] + "\" onclick=\"ewrpt_UpdateSelectAll(this);\"" + checked + " />" + data[i][1] + "<br>"); // ASPXRPT
	}
	html += sb.toString();
	html += "</td></tr></table>";
	html += "</div>";
...
function ewrpt_SelectedEntry(f,name) {
	var elemname = "sel_" + name; // + "[]"; // ASPXRPT
...
function ewrpt_ClearRange(elem) {
	...
	var name = elemname.substring(4, elemname.length); // remove "sel_" // ASPXRPT
	fromname = "rf_" + name;
	toname = "rt_" + name;
