<html>
<head>
	<title></title>
<% If sExport = "" Or sExport = "html" Then %>
<link href="rptcss/Morosos30dias.css" rel="stylesheet" type="text/css">
<% End If %>
<meta name="generator" content="ASP Report Maker v3.0.0.0" />
</head>
<body>
<% If sExport = "" Then %>
<script type="text/javascript">
var EW_REPORT_IMAGES_FOLDER = "rptimages";
</script>
<script src="rptjs/x.js" type="text/javascript"></script>
<div class="ewLayout">
	<!-- header (begin) --><!-- *** Note: Only licensed users are allowed to change the logo *** -->
	<!--<div class="ewHeaderRow"><img src="asprptmkrlogo3.png" alt="" border="0" /></div>-->
	<!-- header (end) -->
	<!-- content (begin) -->
	<!-- navigation -->
	<table cellspacing="0" class="ewContentTable">
		<tr>	
			<!--<td class="ewMenuColumn">
<% 'Server.Execute("rptinc/menu.asp") %>
			</td>-->
			<td class="ewContentColumn">
<% End If %>
