...
%>
	...
	<tr id="r_ANO_MATRICULA">
		<td><span class="aspreportmaker"><%= pagos_vs_llamados.ANO_MATRICULA.FldCaption %></span></td>
		<td></td>
		<td colspan="4"><span class="ewRptSearchOpr">
		...
		<option value="<%= EWRPT_ALL_VALUE %>"<% If ewrpt_MatchedFilterValue(pagos_vs_llamados.ANO_MATRICULA.DropDownValue, EWRPT_ALL_VALUE) Then Response.Write " selected=""selected""" %>><%= ReportLanguage.Phrase("PleaseSelect") %></option>
<%

' Extended Filter
If IsArray(pagos_vs_llamados.ANO_MATRICULA.AdvancedFilters) Then
	cntf = UBound(pagos_vs_llamados.ANO_MATRICULA.AdvancedFilters)+1
Else
	cntf = 0
End If
If IsArray(pagos_vs_llamados.ANO_MATRICULA.DropDownList) Then
	cntd = UBound(pagos_vs_llamados.ANO_MATRICULA.DropDownList)+1
Else
	cntd = 0
End If
totcnt = cntf + cntd
wrkcnt = 0
For i = 0 to cntf-1
	If pagos_vs_llamados.ANO_MATRICULA.AdvancedFilters(i).Enabled Then
%>
		<option value="<%= pagos_vs_llamados.ANO_MATRICULA.AdvancedFilters(i).ID %>"<% If ewrpt_MatchedFilterValue(pagos_vs_llamados.ANO_MATRICULA.DropDownValue, pagos_vs_llamados.ANO_MATRICULA.AdvancedFilters(i).ID) Then Response.Write " selected=""selected""" %>><%= pagos_vs_llamados.ANO_MATRICULA.AdvancedFilters(i).Name %></option>
<%
	End If
	wrkcnt = wrkcnt + 1
Next
For i = 0 to cntd-1
%>
		<option value="<%= pagos_vs_llamados.ANO_MATRICULA.DropDownList(i) %>"<% If ewrpt_MatchedFilterValue(pagos_vs_llamados.ANO_MATRICULA.DropDownValue, pagos_vs_llamados.ANO_MATRICULA.DropDownList(i)) Then Response.Write " selected=""selected""" %>><%= ewrpt_DropDownDisplayValue(pagos_vs_llamados.ANO_MATRICULA.DropDownList(i), "", 0) %></option>
<%
	wrkcnt = wrkcnt + 1
Next
%>
...
<td class="ewTableHeader">
<% If pagos_vs_llamados.Export <> "" Then %>
<%= pagos_vs_llamados.ANO_MATRICULA.FldCaption %>
<% Else %>
	<table cellspacing="0" class="ewTableHeaderBtn"><tr>
<% If pagos_vs_llamados.SortUrl(pagos_vs_llamados.ANO_MATRICULA) = "" Then %>
		<td style="vertical-align: bottom;"><%= pagos_vs_llamados.ANO_MATRICULA.FldCaption %></td>
<% Else %>
		<td class="ewPointer" onmousedown="ewrpt_Sort(event,'<%= pagos_vs_llamados.SortUrl(pagos_vs_llamados.ANO_MATRICULA) %>',1);"><%= pagos_vs_llamados.ANO_MATRICULA.FldCaption %></td><td style="width: 10px;">
		<% If pagos_vs_llamados.ANO_MATRICULA.Sort = "ASC" Then %><img src="rptimages/sortup.gif" width="10" height="9" border="0"><% ElseIf pagos_vs_llamados.ANO_MATRICULA.Sort = "DESC" Then %><img src="rptimages/sortdown.gif" width="10" height="9" border="0"><% End If %></td>
<% End If %>
	</tr></table>
<% End If %>
</td>
...
<span<%= pagos_vs_llamados.ESTACAD.ViewAttributes %>><%= pagos_vs_llamados.ESTACAD.ListViewValue %></span>
		...
		<td<%= pagos_vs_llamados.ANO_MATRICULA.CellAttributes %>>
<span<%= pagos_vs_llamados.ANO_MATRICULA.ViewAttributes %>><%= pagos_vs_llamados.ANO_MATRICULA.ListViewValue %></span>
</td>
...
Class crpagos_vs_llamados_summary
	...
	Sub GetRow(opt)
		...
		If Not rs.Eof Then
			...
			pagos_vs_llamados.ANO_MATRICULA.DbValue = ewrpt_GetValue(rs("ANO_MATRICULA"))
			...
			Val(13) = pagos_vs_llamados.ANO_MATRICULA.CurrentValue
		...
		Else
			...
			pagos_vs_llamados.ANO_MATRICULA.DbValue = ""
	...
	Sub RenderRow()
		...
		Else
			...
			' ANO_MATRICULA
			pagos_vs_llamados.ANO_MATRICULA.ViewValue = pagos_vs_llamados.ANO_MATRICULA.CurrentValue
			pagos_vs_llamados.ANO_MATRICULA.CellAttrs.UpdateAttribute "class", ewrpt_IIf(RecCount Mod 2 <> 1, "ewTableAltRow", "ewTableRow")
			...
			' ANO_MATRICULA
			pagos_vs_llamados.ANO_MATRICULA.HrefValue = ""
		...
		Else
			...
			' ANO_MATRICULA
			Call pagos_vs_llamados.Cell_Rendered(pagos_vs_llamados.ANO_MATRICULA, pagos_vs_llamados.ANO_MATRICULA.CurrentValue, pagos_vs_llamados.ANO_MATRICULA.ViewValue, pagos_vs_llamados.ANO_MATRICULA.ViewAttrs, pagos_vs_llamados.ANO_MATRICULA.CellAttrs, pagos_vs_llamados.ANO_MATRICULA.HrefValue)
	...
	Sub GetExtendedFilterValues()
		...
		wrkSql = ewrpt_BuildReportSql(sSelect, sWhere, "", "", sOrderBy, UserIDFilter, "")
		Call ewrpt_SetDebugMsg("(Extended filter SQL): " & sSql)
		pagos_vs_llamados.ANO_MATRICULA.DropDownList = ewrpt_GetDistinctValues("", wrkSql)
	...
	Function GetExtendedFilter()
		...
		ElseIf Request.QueryString("cmd") = "reset" Then
			...
			' Field ANO_MATRICULA
			Call SetSessionDropDownValue(pagos_vs_llamados.ANO_MATRICULA.DropDownValue, "ANO_MATRICULA")
		...
		Else
			...
			If GetDropDownValue(pagos_vs_llamados.ANO_MATRICULA) Then
				bSetupFilter = True ' Set up filter required
				bRestoreSession = False ' Do not restore from session
			ElseIf Not IsArray(pagos_vs_llamados.ANO_MATRICULA.DropDownValue) Then
				If pagos_vs_llamados.ANO_MATRICULA.DropDownValue <> EWRPT_INIT_VALUE And IsEmpty(Session("sv_pagos_vs_llamados_ANO_MATRICULA")) Then
					bSetupFilter = True ' Set up filter required
				End If
			End If
	...
	Sub LoadDefaultFilters()
		...
		' Field ANO_MATRICULA
		pagos_vs_llamados.ANO_MATRICULA.DefaultDropDownValue = EWRPT_INIT_VALUE
		pagos_vs_llamados.ANO_MATRICULA.DropDownValue = pagos_vs_llamados.ANO_MATRICULA.DefaultDropDownValue
	...
	Sub ShowFilterList()
		...
		If sExtWrk <> "" Or sWrk <> "" Then sFilterList = sFilterList & pagos_vs_llamados.ANO_MATRICULA.FldCaption & "<br>"
		If sExtWrk <> "" Then sFilterList = sFilterList & "&nbsp;&nbsp;" & sExtWrk & "<br>"
		If sWrk <> "" Then sFilterList = sFilterList & "&nbsp;&nbsp;" & sWrk & "<br>"
	...
	Function GetSort()
		...
		If Request.QueryString("cmd").Count > 0 Then
			...
			If sCmd = "resetsort" Then
				...
				pagos_vs_llamados.ANO_MATRICULA.Sort = ""
