...
<% 	
	...
	rsAcceso.Open "select * from IF_ACCESOS_DESC ORDER BY CODLINK ASC", cnIndex
%>
</div>
<div id="content">
    ...
    <div class="content-box column-right main">
        <div class="content-box-header">
			...
			<% if rsAcceso.EOF Then %>
            	<p>No hay proyecto en el sistema.</p>
            <% else %>
            <table>
            	<tr>
                	<th>
             	...
             	<% while not rsAcceso.EOF
					response.Write("<tr>")
						response.Write("<td class='admin_usuario_nombre'>")
							response.Write(rsAcceso("CODLINK"))
						response.Write("</td>")
						response.Write("<td class='admin_usuario_nombre'>")
							response.Write(rsAcceso("PROYECTO"))
						response.Write("</td>")
						response.Write("<td class='admin_usuario_desc'>")
							response.Write(rsAcceso("LINK"))
						response.Write("</td>")
						response.Write("<td class='admin_usuario_desc'>")
							response.Write(rsAcceso("ENLACE"))
						response.Write("</td>")
						response.Write("<td class='admin_usuario_nombre'>")
							response.Write(rsAcceso("EXTENCION"))
						response.Write("</td>")
						response.Write("<td>")
						response.Write("<a href='acceso.asp?a=confirma_proyecto&codlink="&rsAcceso("CODLINK")&"'><img src='images/delete.png' border=0 title='Eliminar'></img></a><a href='acceso.asp?a=modificar_proyecto&codlink="&rsAcceso("CODLINK")&"'><img src='images/edit.gif' border=0 title='Modificar Proyecto'></img></a>")
						response.Write("</td>")
					response.Write("</tr>")
					rsAcceso.MoveNext
					wend
